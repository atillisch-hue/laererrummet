create or replace function public.create_meeting_atomic(
  p_title text,
  p_meeting_type text,
...
begin
  ...
  if p_meeting_type is null or p_meeting_type not in ('Elevmøde','Netværksmøde','Teammøde','Personalemøde','Bestyrelsesmøde','AMR/TR-møde','Andet') then
    raise exception 'Ugyldig mødetype';
  end if;
  if p_starts_at is null or p_ends_at is null or p_ends_at<=p_starts_at then
    raise exception 'Sluttidspunktet skal ligge efter starttidspunktet';
  end if;

  if p_student_id is not null then
    select c.school_id into v_related_school
  ...
  if p_room_id is not null then
    select sr.school_id,sr.name into v_related_school,v_room_name
    from public.school_rooms sr where sr.id=p_room_id and sr.active=true;
  ...
  if v_school_id is null then
    v_school_id:=public.resolve_single_active_school(v_user_id,array['teacher','admin']);
  end if;
  ...
  ) then raise exception 'Eleven tilhører ikke skolen'; end if;
  ...
  select array_agg(distinct x) into v_staff_ids
  from unnest(array_append(coalesce(p_internal_user_ids,array[]::uuid[]),v_user_id)) x
  where x is not null;

  if p_meeting_leader_user_id is not null and not (p_meeting_leader_user_id=any(v_staff_ids)) then
    raise exception 'Mødelederen skal være booket til mødet';
  end if;
  if p_minute_taker_user_id is not null and not (p_minute_taker_user_id=any(v_staff_ids)) then
    raise exception 'Referenten skal være booket til mødet';
  end if;

  if exists(
    select 1
    from public.resource_bookings rb
    left join public.resource_booking_staff rbs on rbs.booking_id=rb.id
    where rb.school_id=v_school_id
      and rb.starts_at<p_ends_at and rb.ends_at>p_starts_at
      and (
        (p_room_id is not null and rb.room_id=p_room_id)
        or rbs.user_id=any(v_staff_ids)
      )
  ) then raise exception 'Tidspunktet kolliderer med et booket lokale eller en booket deltager'; end if;

  insert into public.calendar_meetings(
    title,meeting_type,starts_at,ends_at,location,status,created_by,student_id,
    meeting_leader_user_id,minute_taker_user_id,school_id
  ) values(
    trim(p_title),p_meeting_type,p_starts_at,p_ends_at,v_room_name,'planned',v_user_id,p_student_id,
    p_meeting_leader_user_id,p_minute_taker_user_id,v_school_id
  ) returning id into v_meeting_id;

  insert into public.resource_bookings(title,starts_at,ends_at,room_id,meeting_id,created_by,school_id)
  values(trim(p_title),p_starts_at,p_ends_at,p_room_id,v_meeting_id,v_user_id,v_school_id)
  ...
  select v_meeting_id,x,'invited','internal'
  from (select distinct x from unnest(coalesce(p_internal_user_ids,array[]::uuid[])) x) q
  where x<>v_user_id;

  insert into public.meeting_participants(meeting_id,user_id,attendance_status,access_type)
  select v_meeting_id,x,'invited','guardian'
  from (select distinct x from unnest(coalesce(p_guardian_user_ids,array[]::uuid[])) x) q;
  ...
  if nullif(trim(coalesce(p_external_name,'')),'') is not null then
    ...
    values(v_meeting_id,trim(p_external_name),nullif(trim(coalesce(p_external_role,'')),''),'invited','external');
  end if;
...
revoke all on function public.create_meeting_atomic(text,text,timestamptz,timestamptz,bigint,bigint,uuid[],uuid[],text,text,uuid,uuid) from public,anon;
grant execute on function public.create_meeting_atomic(text,text,timestamptz,timestamptz,bigint,bigint,uuid[],uuid[],text,text,uuid,uuid) to authenticated,service_role;
...
begin
  ...
  if not public.can_edit_meeting(p_meeting_id) then raise exception 'Du har ikke ret til at redigere mødet'; end if;
...
revoke all on function public.save_meeting_notes(bigint,text,text,text) from public,anon;
grant execute on function public.save_meeting_notes(bigint,text,text,text) to authenticated,service_role;
...
as $$
  select
    ...
    case when public.can_access_meeting(cm.id) then cm.title else null end,
    case when public.can_access_meeting(cm.id) then cm.meeting_type else null end,
    case when public.can_access_meeting(cm.id) then cm.starts_at else null end,
    public.can_access_meeting(cm.id)
...
revoke all on function public.my_meeting_actions(boolean) from public,anon;
grant execute on function public.my_meeting_actions(boolean) to authenticated,service_role;

create or replace function public.set_my_meeting_action_completed(p_action_id bigint,p_completed boolean)
returns void
language plpgsql
security definer
set search_path='public','private'
...
begin
  ...
  if not exists(
    ...
    where ma.id=p_action_id
      ...
      and public.is_school_member(cm.school_id)
  ) then raise exception 'Handlingen tilhører ikke dig'; end if;
...
revoke all on function public.set_my_meeting_action_completed(bigint,boolean) from public,anon;
grant execute on function public.set_my_meeting_action_completed(bigint,boolean) to authenticated,service_role;
