...
begin
  ...
  where not exists(
    ...
    where sm.school_id=v_school_id and sm.user_id=t.user_id and sm.active=true
...
create or replace function public.admin_delete_schedule_entry_v2(p_entry_id bigint)
returns void
language plpgsql
security invoker
set search_path=''
...
$$;
revoke all on function public.admin_delete_schedule_entry_v2(bigint) from public,anon;
grant execute on function public.admin_delete_schedule_entry_v2(bigint) to authenticated;
