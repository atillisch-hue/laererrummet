...
begin
  ...
  with inventory as (
    ...
    select 'Faglige elevforsøg',count(*)::bigint,min(x.at),max(x.at),
           'grammatik + læseprøve + staveprøve'
    from (
    ...
    union all
    select 'Møder',count(*)::bigint,min(cm.starts_at),max(cm.starts_at),
           'calendar_meetings inkl. evt. elevkobling og interne noter'
    ...
    select 'Dokumentarkiv',count(*)::bigint,min(sf.created_at),max(sf.created_at),
           'school_files metadata; Storage skal behandles sammen med metadata'
    ...
    select 'Elevsessioner',count(*)::bigint,min(ss.created_at),max(ss.created_at),
           'private.student_sessions; udløbne/tilbagekaldte sessions'
    ...
    select 'Elev-loginforsøg',count(*)::bigint,min(sla.attempted_at),max(sla.attempted_at),
           'private.student_login_attempts; kun sikkerheds-/rate-limit-data'
