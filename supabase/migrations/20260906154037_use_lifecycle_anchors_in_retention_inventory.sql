...
begin
  ...
  with inventory as (
    ...
    union all
    select 'Elevplaner',count(*)::bigint,min(sap.closed_at),max(sap.closed_at),
           'ready_for_policy','closed_at (kun afsluttede/arkiverede planer)','student_action_plans'
    ...
    union all
    select 'Dokumentarkiv',count(*)::bigint,min(sf.archived_at),max(sf.archived_at),
           'ready_for_policy','archived_at (kun arkiverede filer)','school_files metadata; Storage skal behandles sammen med metadata'
