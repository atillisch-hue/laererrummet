...
begin
  ...
  return query
  with inventory(category,record_count,oldest_at,newest_at,readiness,retention_anchor,note) as (
  ...
  select i.category,i.record_count,i.oldest_at,i.newest_at,'decision_required'::text,i.readiness,i.retention_anchor,i.note
  from inventory i order by i.category;
end;
$function$;
