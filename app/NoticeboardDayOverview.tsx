...
export default function NoticeboardDayOverview({selectedDate,onMoveDay,onToday}:Props){
 ...
 const combined=useMemo(()=>[
  ...
  ...meetings.map(meeting=>({kind:"meeting" as const,time:new Date(meeting.starts_at).toLocaleTimeString("da-DK",{hour:"2-digit",minute:"2-digit"}),meeting}))
 ...
 const attendanceDue=(entry:Entry)=>selectedDate<today||(isToday&&entry.start_time.slice(0,5)<=new Date().toLocaleTimeString("da-DK",{hour:"2-digit",minute:"2-digit",hour12:false}));
 ...
 return <article style={overviewCard}>
  ...
  {loading?<div style={quietBox}>Henter din dag…</div>:error&&combined.length===0?<div style={warningBox}>{error}</div>:combined.length===0?<div style={quietBox}>Ingen skema- eller mødepunkter denne dag.</div>:<div style={{display:"grid",gap:6,marginTop:9}}>{combined.slice(0,4).map(row=>row.kind==="meeting"?<div key={`m-${row.meeting.id}`} style={meetingRow}><strong style={{fontSize:11}}>{row.time}{row.meeting.ends_at?`–${new Date(row.meeting.ends_at).toLocaleTimeString("da-DK",{hour:"2-digit",minute:"2-digit"})}`:""} · MØDE</strong><div style={{fontWeight:850,fontSize:13,marginTop:2}}>{row.meeting.title}</div></div>:(()=>{const{entry,mode,assignment}=row.item,lesson=lessonFor(entry.id),unit=lesson?unitFor(lesson.subject_unit_id):null,count=lesson?resourceCount(lesson.id):0;return <div key={`s-${entry.id}-${mode}`} style={{...lessonRow,background:mode==="substitute"?"#edf3ee":mode==="absent"?"#f5ece3":"#f8f8f5"}}><div style={{display:"flex",justifyContent:"space-between",gap:7}}><strong style={{fontSize:11}}>{entry.start_time.slice(0,5)}–{entry.end_time.slice(0,5)} · {entry.subject}</strong>{mode!=="regular"&&<span style={mode==="substitute"?subTag:absentTag}>{mode==="substitute"?"VIKAR":"FRAVÆR"}</span>}</div><div style={{fontWeight:850,fontSize:13,marginTop:2}}>{className(entry.class_id)}{unit?` · ${unit}`:""}</div>{mode==="substitute"&&assignment&&<small style={{display:"block",color:"#667168",marginTop:2}}>for {personName(assignment.absent_teacher_id)}{assignment.substitute_plan?` · ${assignment.substitute_plan}`:""}</small>}{lesson&&mode!=="absent"&&<div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:5}}>{count>0&&<span style={chip}>{count} koblet</span>}{lesson.attendance_checked_at?<span style={chip}>Fravær ført ✓</span>:attendanceDue(entry)?<span style={{...chip,background:"#f4eee0",color:"#75623f"}}>Fravær ikke ført</span>:null}{lesson.status==="completed"&&<span style={chip}>Afsluttet ✓</span>}</div>}{entry.entry_kind==="lesson"&&mode!=="absent"&&<Link href={`/calendar/lesson/${entry.id}?date=${selectedDate}`} style={lessonLink}>Åbn time →</Link>}</div>})())}</div>}
  {combined.length>4&&<small style={{display:"block",marginTop:7,color:"#747c76"}}>+ {combined.length-4} flere punkter i kalenderen</small>}
