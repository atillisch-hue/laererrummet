...
export default function StudentTraining(){
 ...
 const score=questions.filter((question,index)=>answerCorrect(subjectId,question,answers[index]||"")).length;
 const allAnswered=questions.length>0&&questions.every((_,index)=>Boolean(answers[index]?.trim()));
 ...
 function clearRound(clearSeen=true){setAnswers({});setSubmitted(false);setSaveState("");setHelpOpen({});setAvoidKeys([]);if(clearSeen)setSeenKeys([]);setRoundIndex(value=>value+1)}
 function retry(){const current=questions.map(trainingQuestionKey);setSeenKeys(previous=>[...new Set([...previous,...current])]);setAvoidKeys(current);setAnswers({});setSubmitted(false);setSaveState("");setHelpOpen({});setRoundIndex(value=>value+1);window.scrollTo({top:0,behavior:"smooth"})}
 ...
 async function saveResult(){
  ...
  const snapshot=Object.fromEntries(questions.map((question,index)=>[index,{question:question.q,studentAnswer:answers[index],correctAnswer:question.answer,correct:answerCorrect(subjectId,question,answers[index]||""),explanation:question.why,gradeLevel}]));
 ...
 return <main style={shell}><section style={{maxWidth:1000,margin:"0 auto"}}>
  ...
  <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>{gradeAware&&gradeLevel!==null&&<span style={chip}>TILPASSET · {gradeLevel}. klasse · {band}</span>}{subjectId==="matematik"&&<a href="/formelsamling" target="_blank" rel="noreferrer" style={formulaLink}>Formelsamling ↗</a>}</div>
  ...
  {levelId&&<>{questions.length===0?<div style={{...card,marginTop:20}}><strong>Dette niveau er ikke åbent endnu.</strong><p style={muted}>Vælg et niveau, der passer til dit klassetrin.</p></div>:<><div style={{display:"grid",gap:14,marginTop:25}}>{questions.map((item,index)=>{const typed=subjectId==="matematik"&&mathUsesTypedAnswer(skill,item),isCorrect=answerCorrect(subjectId,item,answers[index]||"");return <article key={`${item.q}-${index}`} style={card}><small style={eyebrow}>OPGAVE {index+1} AF {questions.length}</small><h2 style={{...h2,lineHeight:1.35}}>{item.q}</h2>{typed?<input disabled={submitted} inputMode="decimal" value={answers[index]||""} onChange={e=>setAnswers(current=>({...current,[index]:e.target.value}))} placeholder="Skriv dit svar…" style={{...mathInput,borderColor:submitted?(isCorrect?"#5f8068":"#b86b62"):"#d8d5cd",background:submitted?(isCorrect?"#edf5ef":"#fff0ed"):"white"}}/>:<div style={{display:"grid",gap:7}}>{item.options.map(option=>{const chosen=answers[index]===option,correct=submitted&&option===item.answer,wrong=submitted&&chosen&&option!==item.answer;return <button key={option} disabled={submitted} onClick={()=>setAnswers(current=>({...current,[index]:option}))} style={answerButton(chosen,correct,wrong,submitted)}>{option}</button>})}</div>}{subjectId==="matematik"&&!submitted&&<div style={{marginTop:10}}><button type="button" onClick={()=>setHelpOpen(v=>({...v,[index]:!v[index]}))} style={helpButton}>{helpOpen[index]?"Skjul hjælp":"Hjælp mig"}</button>{helpOpen[index]&&<div style={hintBox}><strong>Et lille skub:</strong> {mathHintForSkill(skill)}</div>}</div>}{submitted&&<div style={{marginTop:12,padding:"11px 13px",borderRadius:9,background:isCorrect?"#edf5ef":"#fff7e8",lineHeight:1.55}}><strong>{isCorrect?"Rigtigt ✓":"Ikke helt endnu"}</strong><br/>{!isCorrect&&typed&&<>Det rigtige svar er <strong>{item.answer}</strong>.<br/></>}{item.why}</div>}</article>})}</div>
...
const formulaLink:React.CSSProperties={display:"inline-block",margin:"2px 0 12px",padding:"6px 10px",borderRadius:999,background:"white",border:"1px solid #cfd7d1",color:"#486b59",fontSize:12,fontWeight:900,textDecoration:"none"};
