...
export default function StudentTraining() {
  ...
  function saveResult() {
    ...
    setProgress(next);
    localStorage.setItem(`klassevaerelset-training-${studentCode}`, JSON.stringify(next));
    setSubmitted(true);
  }
  function goBack() {
  ...
  function chooseSubject(nextSubjectId: string) {
    setSubjectId(nextSubjectId); setAreaId(null); setSkill(null); setLevelId(null); resetRun();
  ...
  return <main style={{ minHeight: "100vh", background: "#f5f3ee", padding: "36px 24px 80px", color: "#26342e", fontFamily: "Arial,sans-serif" }}>
    <section style={{ maxWidth: 1000, margin: "0 auto" }}>
      ...
      {levelId ? <>
        <div style={{ display: "grid", gap: 14, marginTop: 25 }}>{questions.map((item,index)=><article key={`${levelId}-${index}`} style={{ background:"#fff",border:"1px solid #d8d5cd",borderRadius:14,padding:22 }}>
          ...
          <div style={{display:"grid",gap:7}}>{item.options.map(option=>{const chosen=answers[index]===option,correct=submitted&&option===item.answer,wrong=submitted&&chosen&&option!==item.answer;return <button key={option} disabled={submitted} onClick={()=>setAnswers(c=>({...c,[index]:option}))} style={{padding:"11px 13px",textAlign:"left",borderRadius:9,border:`2px solid ${correct?"#5f8068":wrong?"#b86b62":chosen?"#526b60":"#e1ddd5"}`,background:correct?"#edf5ef":wrong?"#fff0ed":chosen?"#edf1ec":"#fff",fontWeight:chosen||correct?800:600,cursor:submitted?"default":"pointer"}}>{option}</button>})}</div>
        ...
        <div style={{display:"grid",gap:10,marginTop:24}}>{area.skills.map(areaSkill=>{const levels=freeTrainingQuestions[subjectId]?.[area.id]?.[areaSkill]??{};const availableLevels=Object.keys(levels);const safe=availableLevels.filter(l=>statusFor([subjectId,area.id,areaSkill,l].join("|"))==="Sikker ✓").length;return <button key={areaSkill} disabled={!availableLevels.length} onClick={()=>chooseSkill(areaSkill)} style={{textAlign:"left",background:availableLevels.length?"#fff":"#f8f6f1",border:"1px solid #d8d5cd",borderRadius:13,padding:"17px 19px",cursor:availableLevels.length?"pointer":"default",opacity:availableLevels.length?1:.65,color:"#26342e"}}><strong style={{fontSize:17}}>{areaSkill}</strong><small style={{display:"block",marginTop:7,color:availableLevels.length?"#526b60":"#8a8d88",fontWeight:700}}>{availableLevels.length?`${availableLevels.length} niveau${availableLevels.length===1?"":"er"}${safe?` · ${safe} sikkert`:""} · Vælg →`:"Flere opgaver på vej"}</small></button>})}</div>
        <div style={{marginTop:24,background:"#edf1ec",borderRadius:13,padding:18,lineHeight:1.5}}><strong>Ingen klassetrin her.</strong><br/><span style={{color:"#657068"}}>Du træner det, du har brug for. Niveauet beskriver opgaven – ikke dig.</span></div>
      </>}
    </section>
  </main>;
}
