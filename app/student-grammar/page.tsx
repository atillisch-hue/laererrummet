...
export default function StudentGrammar(){
 ...
 const submit=async()=>{if(!assignment||Object.keys(answers).length!==questions.length)return;setSubmitted(true);setSaving(true);setSaveState("Gemmer resultat…");const code=sessionStorage.getItem("klassevaerelset-student-code");const answerSnapshot=Object.fromEntries(questions.map((q,i)=>[i,{question:q.q,studentAnswer:answers[i],correctAnswer:q.answer,correct:answers[i]===q.answer,explanation:q.why}]));const{data,error}=await supabase.rpc("save_student_grammar_attempt",{p_access_code:code,p_assignment_id:assignment.id,p_answers:answerSnapshot,p_score:score,p_max_score:questions.length});setSaveState(error||!data?.ok?"Resultatet kunne ikke gemmes":"Resultatet er gemt ✓");setSaving(false)};
