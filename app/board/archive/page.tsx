...
export default function BoardArchivePage(){
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",color:"#26342e"}}><section style={{maxWidth:1050,margin:"0 auto",padding:"42px 24px 70px"}}>
  ...
  {schoolId&&<section style={card}><p style={eyebrow}>UPLOAD</p><h2 style={{fontFamily:"Georgia,serif",fontSize:25,margin:"6px 0 8px"}}>Læg et dokument i arkivet</h2><p style={{color:"#707670",margin:"0 0 14px"}}>PDF, Word, Excel, PowerPoint, tekst/CSV og almindelige billeder. Maks. 20 MB.</p><div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}><input ref={inputRef} type="file" onChange={e=>setSelected(e.target.files?.[0]||null)} accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.jpg,.jpeg,.png,.webp"/><button onClick={upload} disabled={!selected||busy} style={{...primary,opacity:!selected||busy?0.55:1}}>{busy?"Arbejder…":"Upload fil →"}</button></div>{selected&&<small style={{display:"block",marginTop:9,color:"#687068"}}>{selected.name} · {fileSize(selected.size)}</small>}</section>}
