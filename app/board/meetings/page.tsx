...
type Decision={id:number;meeting_id:number;decision:string;responsible:string|null;responsible_user_id:string|null;due_date:string|null;completed:boolean};
type BoardMember={user_id:string;display_name:string};
...
export default function BoardMeetingsPage(){
 const[ready,setReady]=useState(false),[meetings,setMeetings]=useState<Meeting[]>([]),[decisions,setDecisions]=useState<Decision[]>([]),[files,setFiles]=useState<BoardFile[]>([]),[boardMembers,setBoardMembers]=useState<BoardMember[]>([]),[schoolId,setSchoolId]=useState<number|null>(null),[busy,setBusy]=useState(false),[message,setMessage]=useState("");
 ...
 const[minutesDraft,setMinutesDraft]=useState<Record<number,string>>({}),[decisionDraft,setDecisionDraft]=useState<Record<number,string>>({}),[responsibleDraft,setResponsibleDraft]=useState<Record<number,string>>({}),[responsibleUserDraft,setResponsibleUserDraft]=useState<Record<number,string>>({}),[dueDraft,setDueDraft]=useState<Record<number,string>>({});
 ...
 const[editingDecision,setEditingDecision]=useState<number|null>(null),[editDecision,setEditDecision]=useState(""),[editResponsible,setEditResponsible]=useState(""),[editResponsibleUser,setEditResponsibleUser]=useState(""),[editDue,setEditDue]=useState("");
 ...
 async function load(){
  const[m,d,f,b]=await Promise.all([
   ...
   supabase.from("board_decisions").select("id,meeting_id,decision,responsible,responsible_user_id,due_date,completed").order("created_at",{ascending:true}),
   ...
   supabase.rpc("board_member_directory")
  ]);
  if(m.error||d.error||f.error||b.error)setMessage("Bestyrelsesmøderne kunne ikke hentes.");
  setMeetings((m.data||[]) as Meeting[]);setDecisions((d.data||[]) as Decision[]);setFiles((f.data||[]) as BoardFile[]);setBoardMembers((b.data||[]) as BoardMember[]);
 ...
 async function addDecision(meetingId:number){const text=(decisionDraft[meetingId]||"").trim();if(!text)return;const userId=responsibleUserDraft[meetingId]||null;const member=boardMembers.find(x=>x.user_id===userId);const{error}=await supabase.from("board_decisions").insert({meeting_id:meetingId,decision:text,responsible_user_id:userId,responsible:member?.display_name||(responsibleDraft[meetingId]||"").trim()||null,due_date:dueDraft[meetingId]||null});if(error){setMessage(error.message);return}setDecisionDraft(x=>({...x,[meetingId]:""}));setResponsibleDraft(x=>({...x,[meetingId]:""}));setResponsibleUserDraft(x=>({...x,[meetingId]:""}));setDueDraft(x=>({...x,[meetingId]:""}));setMessage("Beslutningen er tilføjet ✓");await load()}
 function startDecisionEdit(d:Decision){setEditingDecision(d.id);setEditDecision(d.decision);setEditResponsible(d.responsible_user_id?"":d.responsible||"");setEditResponsibleUser(d.responsible_user_id||"");setEditDue(d.due_date||"");setMessage("")}
 async function saveDecision(id:number){if(!editDecision.trim())return;const member=boardMembers.find(x=>x.user_id===editResponsibleUser);const{error}=await supabase.from("board_decisions").update({decision:editDecision.trim(),responsible_user_id:editResponsibleUser||null,responsible:member?.display_name||editResponsible.trim()||null,due_date:editDue||null}).eq("id",id);if(error)setMessage(error.message);else{setEditingDecision(null);setMessage("Beslutningen er opdateret ✓");await load()}}
 ...
 const meetingCard=(m:Meeting)=>{
  ...
  return <article id={`board-meeting-${m.id}`} key={m.id} style={{...panel,marginBottom:16,scrollMarginTop:20}}>
   {edit?<div style={{display:"grid",gap:12}}><p style={eyebrow}>REDIGÉR MØDE</p><div style={grid}><label style={label}>Titel<input style={input} value={editTitle} onChange={e=>setEditTitle(e.target.value)}/></label><label style={label}>Dato<input style={input} type="date" value={editDate} onChange={e=>setEditDate(e.target.value)}/></label><label style={label}>Tid<input style={input} type="time" value={editTime} onChange={e=>setEditTime(e.target.value)}/></label><label style={label}>Sted<input style={input} value={editLocation} onChange={e=>setEditLocation(e.target.value)}/></label></div><label style={label}>Dagsorden<textarea style={{...input,minHeight:130}} value={editAgenda} onChange={e=>setEditAgenda(e.target.value)}/></label><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button onClick={()=>saveMeeting(m.id)} disabled={busy} style={smallButton}>{busy?"Gemmer…":"Gem møde"}</button><button onClick={()=>setEditingMeeting(null)} style={secondaryButton}>Annullér</button><button onClick={()=>removeMeeting(m.id)} style={deleteButton}>Slet møde</button></div></div>:<>
    ...
    <section style={section}>
     <strong>Beslutninger & opfølgning</strong>
     <div style={{marginTop:10}}>{decisions.filter(d=>d.meeting_id===m.id).map(d=>{const decisionEdit=editingDecision===d.id;return <div key={d.id} style={{padding:"10px 0",borderBottom:"1px solid #eeeae2"}}>{decisionEdit?<div style={{display:"grid",gridTemplateColumns:"minmax(220px,2fr) minmax(170px,1fr) minmax(150px,1fr)",gap:8}}>
      <input aria-label="Beslutning" style={input} value={editDecision} onChange={e=>setEditDecision(e.target.value)} placeholder="Beslutning"/>
      <select aria-label="Ansvarlig bruger" style={input} value={editResponsibleUser} onChange={e=>setEditResponsibleUser(e.target.value)}><option value="">Anden eller ingen bruger</option>{boardMembers.map(member=><option key={member.user_id} value={member.user_id}>{member.display_name}</option>)}</select>
      <input aria-label="Frist" style={input} type="date" value={editDue} onChange={e=>setEditDue(e.target.value)}/>
      {!editResponsibleUser&&<input aria-label="Ansvarlig som fri tekst" style={{...input,gridColumn:"2"}} value={editResponsible} onChange={e=>setEditResponsible(e.target.value)} placeholder="Navn eller ekstern ansvarlig"/>}
      <div style={{gridColumn:"1 / -1",display:"flex",gap:7,flexWrap:"wrap"}}><button onClick={()=>saveDecision(d.id)} style={smallButton}>Gem beslutning</button><button onClick={()=>setEditingDecision(null)} style={secondaryButton}>Annullér</button><button onClick={()=>removeDecision(d.id)} style={deleteButton}>Slet</button></div>
     </div>:<div style={{display:"flex",gap:10,alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap"}}><label style={{display:"flex",gap:10,alignItems:"flex-start",flex:"1 1 300px"}}><input type="checkbox" checked={d.completed} onChange={()=>toggleDecision(d)}/><div><span style={{textDecoration:d.completed?"line-through":"none",fontWeight:700}}>{d.decision}</span>{(d.responsible||d.due_date)&&<small style={{display:"block",color:"#777",marginTop:3}}>{d.responsible?`Ansvarlig: ${d.responsible}`:""}{d.responsible&&d.due_date?" · ":""}{d.due_date?`Frist: ${new Date(d.due_date+"T12:00:00").toLocaleDateString("da-DK")}`:""}</small>}</div></label><div style={{display:"flex",gap:6}}><button onClick={()=>startDecisionEdit(d)} style={secondaryButton}>Redigér</button><button onClick={()=>removeDecision(d.id)} style={deleteButton}>Slet</button></div></div>}</div>})}{decisions.filter(d=>d.meeting_id===m.id).length===0&&<p style={{color:"#8a8f89"}}>Ingen beslutninger registreret endnu.</p>}</div>
     <div style={{display:"grid",gridTemplateColumns:"minmax(220px,2fr) minmax(170px,1fr) minmax(150px,1fr) auto",gap:8,marginTop:14}}>
      <input aria-label="Ny beslutning" style={input} value={decisionDraft[m.id]||""} onChange={e=>setDecisionDraft(x=>({...x,[m.id]:e.target.value}))} placeholder="Beslutning eller opgave"/>
      <select aria-label="Ansvarlig bruger" style={input} value={responsibleUserDraft[m.id]||""} onChange={e=>setResponsibleUserDraft(x=>({...x,[m.id]:e.target.value}))}><option value="">Anden eller ingen bruger</option>{boardMembers.map(member=><option key={member.user_id} value={member.user_id}>{member.display_name}</option>)}</select>
      <input aria-label="Frist" style={input} type="date" value={dueDraft[m.id]||""} onChange={e=>setDueDraft(x=>({...x,[m.id]:e.target.value}))}/>
      <button onClick={()=>addDecision(m.id)} style={{...smallButton,marginTop:0}}>Tilføj</button>
      {!responsibleUserDraft[m.id]&&<input aria-label="Ansvarlig som fri tekst" style={{...input,gridColumn:"2"}} value={responsibleDraft[m.id]||""} onChange={e=>setResponsibleDraft(x=>({...x,[m.id]:e.target.value}))} placeholder="Navn eller ekstern ansvarlig"/>}
     </div>
    </section>
   </>}
  </article>;
