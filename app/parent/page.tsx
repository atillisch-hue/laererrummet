...
type UnitMaterial={id:number;item_type:"post"|"section"|"link"|"material";title:string|null;body:string|null;url:string|null};
type SubjectUnit={id:number;subject_title:string;title:string;driving_question:string|null;summary:string|null;learning_goals:string[];start_date:string|null;end_date:string|null;status:"planned"|"active"|"completed";assignment_ids:number[];materials:UnitMaterial[]};
...
export default function ParentPage(){
 ...
 useEffect(()=>{(async()=>{
  ...
  const initial=requested&&list.some(child=>child.id===requested)?requested:(list[0]?.id||null);
  setChildren(list);setActiveId(initial);if(initial)setChildInUrl(initial);setReady(true);
 })()},[]);
 ...
 const selectChild=(id:number)=>{setActiveId(id);setOpenAssignmentId(null);setOpenUnitId(null);setChildInUrl(id)};
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",color:"#26342e"}}>
  <section style={shell}>
   ...
   {!children.length?<section style={{...card,marginTop:22}}>Skolen mangler at knytte et barn til din aktive forældrekonto.</section>:<>
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:24}}>{children.map(child=><button key={child.id} onClick={()=>selectChild(child.id)} style={{padding:"10px 14px",borderRadius:9,border:active?.id===child.id?"1px solid #365044":"1px solid #d8d5cd",background:active?.id===child.id?"#365044":"white",color:active?.id===child.id?"white":"#27352d",fontWeight:850,cursor:"pointer"}}>{child.name}{child.class_name?` · ${child.class_name}`:""}</button>)}</div>
    ...
    {visibleUnits.length>0&&<section style={{...card,marginTop:18,background:"#eef2ed"}}>
     ...
     <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(270px,1fr))",gap:10,marginTop:14}}>{visibleUnits.map(unit=>{const open=openUnitId===unit.id,unitAssignments=unit.assignment_ids.map(id=>active?.assignments.find(a=>a.id===id)).filter(Boolean) as Assignment[];return <article key={unit.id} style={{background:"white",border:"1px solid #d9e0da",borderRadius:11,overflow:"hidden"}}><button type="button" onClick={()=>setOpenUnitId(open?null:unit.id)} style={{width:"100%",border:0,background:"transparent",padding:13,textAlign:"left",color:"inherit",cursor:"pointer"}}><div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"start"}}><div><div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}><small style={unitChip}>{unit.subject_title.toUpperCase()}</small><small style={{...unitChip,background:unit.status==="active"?"#e3ece5":"#eeeae1",color:unit.status==="active"?"#486252":"#76694f"}}>{unitStatus[unit.status]}</small>{(unitAssignments.length+unit.materials.length)>0&&<small style={unitChip}>{unitAssignments.length+unit.materials.length} dele</small>}</div><strong style={{display:"block",fontFamily:"Georgia,serif",fontSize:20,marginTop:8}}>{unit.title}</strong>{unit.driving_question&&<span style={{display:"block",fontSize:13,lineHeight:1.45,color:"#59655e",marginTop:5}}>“{unit.driving_question}”</span>}<small style={{display:"block",color:"#7a817b",marginTop:7}}>{unitPeriod(unit)}</small></div><strong style={{color:"#526b60",fontSize:12}}>{open?"Luk ↑":"Se mere ↓"}</strong></div></button>{open&&<div style={{padding:"0 13px 13px",borderTop:"1px solid #e5e1d9"}}>{unit.summary&&<p style={{lineHeight:1.55,color:"#46534c",margin:"11px 0"}}>{unit.summary}</p>}{unit.learning_goals.length>0&&<><p style={{...eyebrow,marginTop:11}}>LÆRINGSMÅL</p><ul style={{paddingLeft:20,margin:"7px 0 0",color:"#46534c",lineHeight:1.55}}>{unit.learning_goals.map(goal=><li key={goal}>{goal}</li>)}</ul></>}{unitAssignments.length>0&&<div style={{marginTop:14}}><p style={eyebrow}>OPGAVER I FORLØBET</p><div style={{display:"grid",gap:7,marginTop:7}}>{unitAssignments.map(a=><div key={a.id} style={unitWorkCard}><strong>{a.title}</strong><small style={{display:"block",color:"#717973",marginTop:2}}>{a.type}</small>{a.instructions&&<p style={{whiteSpace:"pre-wrap",lineHeight:1.45,color:"#566159",fontSize:12,margin:"6px 0 0"}}>{a.instructions}</p>}</div>)}</div></div>}{unit.materials.length>0&&<div style={{marginTop:14}}><p style={eyebrow}>MATERIALER I FORLØBET</p><div style={{display:"grid",gap:7,marginTop:7}}>{unit.materials.map(material=><div key={material.id} style={unitWorkCard}><small style={{display:"block",fontWeight:900,color:"#718077"}}>{materialLabel[material.item_type].toUpperCase()}</small><strong style={{display:"block",marginTop:2}}>{material.title||materialLabel[material.item_type]}</strong>{material.body&&<p style={{lineHeight:1.45,color:"#566159",fontSize:12,margin:"5px 0 0"}}>{material.body.length>260?`${material.body.slice(0,260)}…`:material.body}</p>}{material.url&&<a href={material.url} target="_blank" rel="noreferrer" style={{display:"inline-block",marginTop:5,color:"#486b59",fontWeight:850,fontSize:12}}>Åbn →</a>}</div>)}</div></div>}{unitAssignments.length===0&&unit.materials.length===0&&<p style={{fontSize:12,color:"#777168",margin:"12px 0 0"}}>Læreren har ikke delt konkrete opgaver eller materialer i dette forløb endnu.</p>}<small style={{display:"block",marginTop:11,color:"#777168"}}>Her vises kun lærerens delte plan og indhold. Elevens kladde, svar og feedback vises ikke.</small></div>}</article>})}</div>
    </section>}
...
const unitWorkCard:React.CSSProperties={padding:"9px 10px",border:"1px solid #e2ded6",borderRadius:8,background:"#faf9f6"};