...
type PublishResult={published_version_id:number;published_name:string;effective_from:string;new_draft_version_id:number;warnings?:PublishWarnings};
...
export default function PublishSchedulePanel({year,draft,published,onPublished}:Props){
 ...
 async function publish(){
  ...
  if(e){setError(e.message||"Skemaet kunne ikke publiceres.");setPublishing(false);return}
  ...
  setMessage(`Skemaet er publiceret fra ${dateLabel(result.effective_from||publishDate)} ✓ Ny arbejdskladde er oprettet automatisk.${warnings.length?` Ledelsesadvarsler: ${warnings.join(" · ")}.`:""}`);
  await onPublished();
  setPublishing(false);
 }
 ...
 return <section style={{marginTop:14,padding:15,borderRadius:11,background:"#edf3ee",border:"1px solid #cfdcd2"}}>
  ...
  {error&&<div style={{marginTop:11,padding:"10px 11px",borderRadius:8,background:"#fff0ed",color:"#7b332b",fontWeight:800,fontSize:13}}>{error}</div>}
  ...
  <small style={{display:"block",marginTop:10,color:"#69746d",lineHeight:1.45}}>Publicering stoppes automatisk ved skemakonflikter, uafklarede fagkoblinger eller manglende/inaktiv bemanding. Afvigelser i norm og undervisningsminutter vises som ledelsesadvarsler og kan vurderes fagligt.</small>
 </section>;
}
