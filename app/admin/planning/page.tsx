...
const categories=[
 ["preparation","Forberedelse"],["meeting","Møder"],["duty","Tilsyn / vagter"],["leadership","Ledelse"],["amr","AMR"],["tr","TR"],["reading_specialist","Læsevejleder"],["project","Projekt"],["administration","Administration"],["other","Andet"]
] as const;
...
export default function PlanningPage(){
 ...
 async function load(targetSchoolId?:number|null){
  ...
  if(yErr||!y){setError(yErr?.message||"Skolen har ikke et aktivt skoleår endnu.");return}
  ...
  if(classRows.length){const{sdata,error:sErr}=await (async()=>{const res=await supabase.from("class_subjects").select("id,class_id,title").in("class_id",classRows.map(c=>c.id)).eq("active",true).order("title");return{sdata:res.data,error:sErrValue(res.error)}})();if(error){}
   subjectRows=(sdata||[]) as ClassSubject[];
  }
  ...
  const staffRows=((dRes.data||[]) as StaffDirectory[]).map(d=>({
   ...d,display_name:profiles.get(d.user_id)?.display_name||d.abbreviation||"Medarbejder",active:profiles.get(d.user_id)?.active!==false,work:work.get(d.user_id),allocated_minutes:allocated.get(d.user_id)||0
  })).filter(x=>x.active).sort((a,b)=>(a.abbreviation||a.display_name).localeCompare(b.abbreviation||b.display_name,"da"));
 ...
 return <main style={{minHeight:"100vh",background:"#f5f2ea",color:"#26342e"}}>
  ...
  <section style={{maxWidth:1180,margin:"auto",padding:"34px 24px 80px"}}>
   ...
   <div style={{display:"grid",gridTemplateColumns:"minmax(0,1.35fr) minmax(300px,.65fr)",gap:18,marginTop:18,alignItems:"start"}}>
   ...
   <div style={{display:"grid",gridTemplateColumns:"minmax(0,1.2fr) minmax(300px,.8fr)",gap:18,marginTop:18,alignItems:"start"}}>
...
function sErrValue(e:any){return e}
