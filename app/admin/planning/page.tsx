...
export default function PlanningPage(){
 ...
 const currentVersion=draft||published||versions[0];
 ...
 return <main style={{minHeight:"100vh",background:"#f5f2ea",color:"#26342e"}}>
  ...
  <section style={{maxWidth:1180,margin:"auto",padding:"34px 24px 80px"}}>
   ...
   <section style={{...card,marginTop:18}}><div style={{display:"flex",justifyContent:"space-between",gap:15,alignItems:"start",flexWrap:"wrap"}}><div><small style={eyebrow}>SKEMAVERSION</small><h2 style={heading}>{currentVersion?.name||"Ingen skemaversion"}</h2><p style={muted}>{currentVersion?`${currentVersion.status==="draft"?"Arbejdskladde":statusLabel(currentVersion.status)}${currentVersion.effective_from?` · gælder fra ${dateText(currentVersion.effective_from)}`:""}`:"Opret en skemaversion før skemalægningen starter."}</p></div><Link href="/admin/schedule" style={{...secondary,textDecoration:"none"}}>Åbn skema →</Link></div>{versions.length>0&&<div style={{display:"grid",gap:7,marginTop:12}}>{versions.map(v=><div key={v.id} style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",padding:"10px 12px",background:v.status==="draft"?"#edf3ee":"#f7f5ef",borderRadius:9}}><div><strong>{v.name}</strong><small style={{display:"block",color:"#727a73"}}>{statusLabel(v.status)}{v.effective_from?` · ${dateText(v.effective_from)}`:""}{v.effective_to?` – ${dateText(v.effective_to)}`:""}</small></div>{v.status==="draft"&&<span style={{fontSize:11,fontWeight:850,color:"#365844",background:"white",padding:"6px 9px",borderRadius:999}}>Arbejdsversion</span>}{v.status==="published"&&<span style={{fontSize:11,fontWeight:850,color:"white",background:"#486b59",padding:"6px 9px",borderRadius:999}}>Gældende</span>}</div>)}</div>}<PublishSchedulePanel year={year} draft={draft} published={published} onPublished={()=>load()}/></section>

   <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:18,marginTop:18,alignItems:"start"}}>
    <section style={card}><small style={eyebrow}>PERSONALERESSOURCER</small><h2 style={heading}>Norm og øvrige opgaver</h2><p style={muted}>Undervisningen beregnes fra den aktuelle arbejdskladde. Her ligger de øvrige timer/akkorder, som også skal bruge af årsrammen.</p><div style={{overflowX:"auto",marginTop:14}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:610}}><thead><tr>{["Medarbejder","Ansættelse","Årsramme","Øvrige opgaver"].map(x=><th key={x} style={th}>{x}</th>)}</tr></thead><tbody>{staff.map(s=><tr key={s.user_id}><td style={td}><strong>{s.abbreviation||"—"} · {s.display_name}</strong><small style={{display:"block",color:"#7a807a"}}>{s.personnel_group}</small></td><td style={td}>{s.work?.employment_percent?`${s.work.employment_percent}%`:"—"}</td><td style={td}>{s.work?hours(s.work.annual_target_minutes):"Ikke sat"}</td><td style={td}>{hours(s.allocated_minutes)}</td></tr>)}</tbody></table>{!staff.length&&<p style={muted}>Der er endnu ingen personaleprofiler at planlægge på.</p>}</div><Link href="/calendar" style={{display:"inline-block",marginTop:13,color:"#365044",fontWeight:850}}>Åbn årsnorm & arbejdstid →</Link></section>
   ...
   <section style={{...card,marginTop:18,background:"#eef2ec"}}><small style={eyebrow}>SAMME DATAGRUNDLAG</small><h2 style={heading}>Skema, norm og drift hænger nu sammen</h2><p style={{...muted,maxWidth:900}}>Konsekvensberegningen følger arbejdskladden. Når den publiceres, bliver den låst som historik og begynder at drive Kalender, Min dag, forældreskema, vikarer og mødebooking fra den valgte dato. Den automatisk oprettede næste kladde kan derefter ændres uden at påvirke den gældende version.</p></section>
  </section>
 </main>;
}
