...
type RetentionRow={category:string;record_count:number;oldest_at:string|null;newest_at:string|null;policy_status:string;note:string};
...
export default function SecurityHistoryPage(){
 const[ready,setReady]=useState(false),[events,setEvents]=useState<AuditEvent[]>([]),[users,setUsers]=useState<DirectoryUser[]>([]),[students,setStudents]=useState<Student[]>([]),[retention,setRetention]=useState<RetentionRow[]>([]),[error,setError]=useState("");
 ...
 useEffect(()=>{let live=true;(async()=>{
  ...
  const schoolId=membershipRes.data?.school_id?Number(membershipRes.data.school_id):null;
  const[auditRes,userRes,studentRes,retentionRes]=await Promise.all([
   ...
   schoolId?supabase.rpc("admin_retention_inventory",{p_school_id:schoolId}):Promise.resolve({data:[],error:null})
  ]);
  if(!live)return;
  ...
  setRetention((retentionRes.data||[]) as RetentionRow[]);
  setReady(true);
 })();return()=>{live=false}},[]);
 ...
 return <main style={{minHeight:"100vh",background:"#f5f2ea",color:"#26342e"}}>
  ...
  <section style={shell}>
   ...
   <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:10,marginTop:10}}>{retention.map(row=><article key={row.category} style={card}><div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"start"}}><strong style={{fontFamily:"Georgia,serif",fontSize:18}}>{row.category}</strong><span style={{fontSize:11,fontWeight:900,borderRadius:999,padding:"4px 7px",background:"#fff3df",color:"#7a5a24"}}>FRIST MANGLER</span></div><div style={{fontSize:28,fontWeight:900,marginTop:12}}>{row.record_count}</div><small style={{display:"block",color:"#687068",marginTop:2}}>poster</small><dl style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:"5px 9px",fontSize:12,margin:"13px 0 0",color:"#667068"}}><dt>Ældste</dt><dd style={{margin:0,fontWeight:800}}>{formatDate(row.oldest_at)}</dd><dt>Nyeste</dt><dd style={{margin:0,fontWeight:800}}>{formatDate(row.newest_at)}</dd></dl></article>)}</div>
