...
export default function Schedule(){
 ...
 async function saveEntryEdit(){
  ...
  const{error}=await supabase.rpc("admin_update_schedule_entry",{
   ...
   p_recurrence_pattern:editRecurrence
  });
 ...
 return <main style={{minHeight:"100vh",background:"#f5f2ea",color:"#26342e"}}>
  ...
  <section style={{maxWidth:1100,margin:"auto",padding:"42px 24px"}}>
   <p className="eyebrow">SKEMA</p><h1 style={{fontFamily:"Georgia,serif",fontSize:40}}>Klassernes skema</h1><p style={{color:"#687068"}}>Undervisning, samling, pauser og vagter kan ligge i samme skema. Brikker kan køre hver uge eller kun i ulige/lige uger.</p>{msg&&<div style={{padding:12,background:"#e7eee9",borderRadius:9,margin:"18px 0"}}>{msg}</div>}
   ...
   <form onSubmit={add} style={{background:"white",padding:22,border:"1px solid #ddd9d0",borderRadius:14,display:"grid",gap:14}}>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:10}}>
     ...
     <select value={weekday} onChange={e=>setWeekday(Number(e.target.value))} style={field}>{days.map((d,i)=><option key={d} value={i+1}>{d}</option>)}</select>
   ...
   <div style={{marginTop:28,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(205px,1fr))",gap:14}}>{days.map((d,i)=><section key={d} style={{background:"white",border:"1px solid #ddd9d0",borderRadius:12,padding:16}}><h2 style={{fontFamily:"Georgia,serif",fontSize:20}}>{d}</h2>{shown.filter(x=>x.weekday===i+1).map(x=>{const tids=links.filter(l=>l.schedule_entry_id===x.id).map(l=>teachers.find(t=>t.id===l.teacher_id)).filter((t):t is Teacher=>Boolean(t));const editing=editingId===x.id;return <div key={x.id} style={{background:tone(x.subject),borderRadius:8,padding:10,marginTop:8}}>{editing?<div style={{display:"grid",gap:8}}>
    ...
    <select value={editWeekday} onChange={e=>setEditWeekday(Number(e.target.value))} style={field}>{days.map((dayName,di)=><option key={dayName} value={di+1}>{dayName}</option>)}</select>
    ...
    <div style={{display:"flex",gap:6,alignItems:"center",justifyContent:"space-between",flexWrap:"wrap"}}><small style={{fontWeight:900,color:"#667168"}}>{kindLabel(x.entry_kind).toUpperCase()}</small><small style={{fontWeight:900,color:"#5e6d64",background:"rgba(255,255,255,.55)",borderRadius:999,padding:"3px 6px"}}>{recurrenceLabel(x.recurrence_pattern).toUpperCase()}</small></div>
    <strong style={{display:"block",marginTop:4}}>{x.start_time.slice(0,5)}–{x.end_time.slice(0,5)} · {x.subject}</strong><small style={{display:"block",color:"#555"}}>{tids.map(initials).join(" + ")}{x.room?` · ${x.room}`:""}</small><div style={{display:"flex",gap:6,marginTop:7}}><button type="button" onClick={()=>startEditEntry(x)} style={smallButton}>Redigér</button><button type="button" onClick={()=>remove(x.id)} style={{...smallButton,color:"#8a3c34"}}>Slet</button></div>
   </>}</div>})}{!shown.some(x=>x.weekday===i+1)&&<small style={{color:"#888"}}>Ingen timer eller aktiviteter</small>}</section>)}</div>
  </section>
 </main>;
}
