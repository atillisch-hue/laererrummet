...
const teachingItems=[
 {label:"I dag",href:"/noticeboard",roots:["/noticeboard"]},
 {label:"Klasser",href:"/teacher-dashboard",roots:["/teacher-dashboard","/students","/student-profile","/teacher-overview","/create-assignment","/grammar","/math","/danish","/formelsamling"]},
 ...
 {label:"Forberedelse",href:"/preparation",roots:["/preparation"]}
];
...
const protectedRoots=[...teachingItems.flatMap(x=>x.roots),"/admin","/board","/parent","/account"];
const mainTabs=new Set(["/noticeboard","/teacher-dashboard","/calendar","/teacher-room","/preparation"]);
...
export default function DashboardNav(){
 ...
 const[ready,setReady]=useState(false),[teacher,setTeacher]=useState(false),[staff,setStaff]=useState(false),[leader,setLeader]=useState(false),[admin,setAdmin]=useState(false),[parent,setParent]=useState(false),[board,setBoard]=useState(false),[email,setEmail]=useState("");
 ...
 useEffect(()=>{let active=true;const sync=async()=>{const{data}=await supabase.auth.getSession();if(!active)return;const user=data.session?.user;setTeacher(!!user&&hasRole(user,"teacher"));setStaff(!!user&&hasRole(user,"staff"));setLeader(!!user&&hasRole(user,"leader"));setAdmin(!!user&&hasRole(user,"admin"));setParent(!!user&&hasRole(user,"parent"));setBoard(!!user&&hasRole(user,"board"));setEmail(user?.email||"");setReady(!!user)};sync();const{data:listener}=supabase.auth.onAuthStateChange(()=>sync());return()=>{active=false;listener.subscription.unsubscribe()}},[]);
 ...
 const roleLabel=starts(pathname,"/account")?"Konto":starts(pathname,"/admin")?(admin?"Admin":"Ledelse"):starts(pathname,"/parent")?"Forælder":starts(pathname,"/board")?"Bestyrelse":teacher?"Lærer":leader?"Ledelse":staff?"Medarbejder":admin?"Admin":"Bruger";
 ...
 return <>
  <nav aria-label="Primær navigation" style={{position:"sticky",top:0,zIndex:50,background:"rgba(245,242,234,.97)",borderBottom:"1px solid #ddd9d0",backdropFilter:"blur(10px)"}}><div style={{maxWidth:1280,margin:"0 auto",padding:"8px 14px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><Link href={homeHref} style={{display:"inline-flex",alignItems:"center",gap:6,color:"#26342e",textDecoration:"none",fontWeight:900,whiteSpace:"nowrap",fontSize:14,paddingRight:2}}><span aria-hidden="true">✦</span> Klasseværelset</Link>{employeeAccess&&<div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap",flex:"1 1 auto",minWidth:0}}>{visibleItems.map(item=>{const active=item.roots.some(root=>starts(pathname,root));return <Link key={item.href} href={item.href} aria-current={active?"page":undefined} style={{display:"inline-flex",alignItems:"center",padding:"8px 9px",borderRadius:8,textDecoration:"none",fontWeight:800,fontSize:12,whiteSpace:"nowrap",border:active?"1px solid #486b59":"1px solid transparent",background:active?"#486b59":"transparent",color:active?"white":"#425249"}}>{item.label}</Link>})}</div>}<div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>{board&&<Link href="/board" style={roleButton(starts(pathname,"/board"))}>Bestyrelse</Link>}{parent&&<Link href="/parent" style={roleButton(starts(pathname,"/parent"))}>Forælder</Link>}{leader&&!admin&&<Link href="/admin" style={roleButton(starts(pathname,"/admin"))}>Ledelse</Link>}{admin&&<Link href="/admin" style={roleButton(starts(pathname,"/admin"))}>Admin</Link>}{(leader||admin)&&<Link href="/account/security" style={roleButton(starts(pathname,"/account/security"))}>Kontosikkerhed</Link>}<span title={email||roleLabel} style={{display:"inline-flex",alignItems:"center",padding:"6px 8px",borderRadius:999,background:"#e8ece8",color:"#526159",fontSize:10,fontWeight:900,whiteSpace:"nowrap"}}>{roleLabel}</span><button onClick={logout} style={{display:"inline-flex",alignItems:"center",padding:"7px 9px",borderRadius:8,fontWeight:900,fontSize:11,whiteSpace:"nowrap",border:"1px solid #cfcac0",color:"#526159",background:"#fff",cursor:"pointer"}}>Log ud</button></div></div></nav>
  {teachingAccess&&subjectRoom&&<nav aria-label={`${subjectRoom.subjectName} værktøjer`} style={{background:"#edf1ec",borderBottom:"1px solid #d8ddd7"}}><div style={{maxWidth:1050,margin:"0 auto",padding:"10px 24px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><span style={{fontSize:12,fontWeight:900,color:"#64746b",marginRight:5}}>I dette fag</span>{isMathRoom&&<><Link href={`/students/class-math-profile?class=${subjectRoom.classId}`} style={subjectToolButton(false)}>Fagligt overblik</Link><Link href={`/math?class=${subjectRoom.classId}`} style={subjectToolButton(false)}>Træning & progression</Link><Link href="/formelsamling" style={subjectToolButton(false)}>Formelsamling</Link></>}{isDanishRoom&&<><Link href={`/danish?class=${subjectRoom.classId}`} style={subjectToolButton(false)}>Dansk-værktøjer</Link><Link href={`/students/class-learning-profile?class=${subjectRoom.classId}`} style={subjectToolButton(false)}>Fagligt overblik</Link><Link href={`/grammar?mode=assign&class=${subjectRoom.classId}&room=${subjectRoom.roomId}`} style={subjectToolButton(false)}>Grammatik & sprog</Link></>}<Link href={`/students/subjects/${subjectRoom.roomId}/units`} style={subjectToolButton(false)}>Forløb & årsplan</Link><Link href={`/teacher-overview?class=${subjectRoom.classId}&subject=${subjectRoom.roomId}`} style={subjectToolButton(false)}>Opgaver & besvarelser</Link><Link href={`/students/subjects/${subjectRoom.roomId}/users`} style={subjectToolButton(false)}>Deltagere</Link><Link href={`/create-assignment?class=${subjectRoom.classId}&subject=${subjectRoom.roomId}`} style={subjectToolButton(true)}>+ Ny opgave</Link></div></nav>}
  {teachingAccess&&subjectRoom&&<style>{`main > header:first-child { padding-top:18px !important;padding-bottom:18px !important;} main > header:first-child h1 {font-size:34px !important;} main > header:first-child p {line-height:1.4 !important;}`}</style>}
