...
type AgendaItem=
 ...
 |{kind:"block";sort:number;entry:ScheduleEntry}
...
const blockLabel=(kind:EntryKind)=>({assembly:"SAMLING",break:"PAUSE",duty:"GÅRDVAGT",other:"AKTIVITET",lesson:"LEKTION"}[kind]);
const blockTone=(kind:EntryKind)=>kind==="assembly"?{background:"#fff5df",border:"#ead5a5",color:"#725e36"}:kind==="break"?{background:"#f2f1ed",border:"#dfddd6",color:"#716e67"}:kind==="duty"?{background:"#e9f0e9",border:"#cfddcf",color:"#516a55"}:{background:"#f3f1eb",border:"#ddd8cc",color:"#665f52"};
...
export default function SchoolCalendarWidget({selectedDate,onSelectDate,markedDates=[],meetings=[]}:{selectedDate?:string;onSelectDate?:(date:string)=>void;markedDates?:string[];meetings?:CalendarMeeting[]}){
 ...
 const hasOnlineContent=(entry:ScheduleEntry)=>{
 ...
 const attendanceState=(entry:ScheduleEntry)=>{
  ...
  const now=new Date();const[hour,minute]=entry.start_time.split(":").map(Number);const start=new Date();start.setHours(hour,minute,0,0);
 ...
 const agendaItems:AgendaItem[]=[
  ...
  ...dayBlocks.map(entry=>({kind:"block" as const,sort:new Date(`${activeDate}T${entry.start_time}`).getTime(),entry})),
 ...
 return <div style={{display:"grid",gridTemplateColumns:pathname==="/calendar"?"minmax(280px,390px) minmax(0,1fr)":"1fr",gap:14,alignItems:"start"}}>
  ...
  {pathname==="/calendar"&&<section style={{background:"white",border:"1px solid #ddd9d0",borderRadius:15,padding:18}}>
   ...
   <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}><span style={chip}>Undervisning {dayLessons.length}</span>{dayBlocks.length>0&&<span style={chip}>Arbejdsblokke {dayBlocks.length}</span>}<span style={chip}>Møder {dayMeetings.length}</span><span style={chip}>Deadlines {dayTasks.length}</span>{daySubs.length>0&&<span style={chip}>Vikarændringer {daySubs.length}</span>}</div>
   {agendaItems.length===0?<p style={{color:"#737b75",fontSize:13,margin:"10px 0 0"}}>{activeClosure?"Ingen planlagte arbejdsobjekter denne dag.":"Ingen undervisning, arbejdsblokke, møder eller deadlines i dit arbejdsflow denne dag."}</p>:<div style={{display:"grid",gap:8}}>
    {agendaItems.map(item=>{
     if(item.kind==="lesson"){
      const entry=item.entry,change=daySubs.find(x=>x.schedule_entry_id===entry.id),absent=change?.absent_teacher_id===userId,substitute=change?.substitute_teacher_id===userId,attendance=attendanceState(entry),online=hasOnlineContent(entry);
      return <Link key={`lesson-${entry.id}`} href={`/calendar/lesson/${entry.id}?date=${activeDate}`} style={{textDecoration:"none",color:"inherit",border:"1px solid #e2ded5",borderRadius:10,padding:"11px 12px",background:absent?"#f5eadc":substitute?"#e7eee9":"#faf9f6",display:"block"}}><div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"start"}}><strong style={{fontSize:13}}>{entry.start_time.slice(0,5)}–{entry.end_time.slice(0,5)} · {entry.subject}</strong><div style={{display:"flex",gap:5,alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}>{online&&<small style={onlineTag}>ONLINE INDHOLD ✓</small>}<small style={kindTag}>LEKTION</small>{entry.recurrence_pattern!=="weekly"&&<small style={kindTag}>{recurrenceLabel(entry.recurrence_pattern).toUpperCase()}</small>}{(absent||substitute)&&<small style={{fontWeight:900,color:"#6c5d43"}}>{absent?"FRAVÆR":"VIKAR"}</small>}</div></div><small style={{display:"block",color:"#717771",marginTop:3}}>{className(entry.class_id)}{entry.room?` · ${entry.room}`:""}</small><div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap",marginTop:7}}><span style={attendance.tone==="done"?attendanceDone:attendance.tone==="missing"?attendanceMissing:attendanceFuture}>{attendance.label}</span>{attendance.time&&<small style={{color:"#5c6f63",fontWeight:800}}>kl. {attendance.time}</small>}</div>{substitute&&change?.substitute_plan&&<small style={{display:"block",marginTop:6,color:"#52675a"}}>Vikarplan: {change.substitute_plan}</small>}{absent&&change&&<small style={{display:"block",marginTop:6,color:"#765f42"}}>Vikar: {staffName(change.substitute_teacher_id)}</small>}</Link>;
     }
     if(item.kind==="block"){
...
const onlineTag:React.CSSProperties={...kindTag,background:"#dfece3",color:"#41614d",border:"1px solid #c5d9ca"};
