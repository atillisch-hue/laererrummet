 ...
 return <div style={{display:"grid",gap:14}}>
  ...
  {pathname==="/calendar"&&<section style={{background:"white",border:"1px solid #ddd9d0",borderRadius:15,padding:18}}>
   ...
   {dayLessons.length===0&&dayTasks.length===0?<p style={{color:"#737b75",fontSize:13,margin:"10px 0 0"}}>{activeClosure?"Ingen undervisning i det faste skema denne dag.":"Ingen undervisning eller deadlines i dit arbejdsflow denne dag."}</p>:<div style={{display:"grid",gap:8}}>
    {dayLessons.map(entry=>{const change=daySubs.find(x=>x.schedule_entry_id===entry.id);const absent=change?.absent_teacher_id===userId;const substitute=change?.substitute_teacher_id===userId;const attendance=attendanceState(entry);return <Link key={`lesson-${entry.id}`} href={`/calendar/lesson/${entry.id}?date=${activeDate}`} style={{textDecoration:"none",color:"inherit",border:"1px solid #e2ded5",borderRadius:10,padding:"11px 12px",background:absent?"#f5eadc":substitute?"#e7eee9":"#faf9f6",display:"block"}}><div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"start"}}><strong style={{fontSize:13}}>{entry.start_time.slice(0,5)}–{entry.end_time.slice(0,5)} · {entry.subject}</strong>{(absent||substitute)&&<small style={{fontWeight:900,color:"#6c5d43"}}>{absent?"FRAVÆR":"VIKAR"}</small>}</div><small style={{display:"block",color:"#717771",marginTop:3}}>{className(entry.class_id)}{entry.room?` · ${entry.room}`:""}</small><div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap",marginTop:7}}><span style={attendance.tone==="done"?attendanceDone:attendance.tone==="missing"?attendanceMissing:attendanceFuture}>{attendance.label}</span>{attendance.time&&<small style={{color:"#5c6f63",fontWeight:800}}>kl. {attendance.time}</small>}</div>{substitute&&change?.substitute_plan&&<small style={{display:"block",marginTop:6,color:"#52675a"}}>Vikarplan: {change.substitute_plan}</small>}{absent&&change&&<small style={{display:"block",marginTop:6,color:"#765f42"}}>Vikar: {staffName(change.substitute_teacher_id)}</small>}</Link>})}
