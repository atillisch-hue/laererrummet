...
type Room={id:number;school_id:number;class_id:number;subject_id:number;title:string|null};
...
export default function ActiveSubjectUnits(){
 ...
 useEffect(()=>{let active=true;(async()=>{
  ...
  let roomQuery=supabase.from("class_subjects").select("id,school_id,class_id,subject_id,title").eq("active",true);
 ...
 const createUnit=async()=>{
  ...
  const room=rooms.find(x=>x.id===roomId),option=roomOptions.find(x=>x.id===roomId),href=`/students/subjects/${roomId}/units?new=${data.id}`;
  if(room)await rememberWork({schoolId:room.school_id,objectType:"subject_unit",objectKey:data.id,title:form.title,subtitle:option?.label||null,href});
