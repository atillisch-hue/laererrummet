 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",padding:"38px 24px 80px",color:"#26342e"}}><section style={{maxWidth:1100,margin:"auto"}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"1fr 1.25fr",gap:18,alignItems:"start"}}>
   ...
   <section style={card}><h2 style={{fontFamily:"Georgia,serif",marginTop:0}}>Vikardækning</h2>{coverage.length?coverage.map(({a,e})=>{const ownAbsence=a.absent_teacher_id===uid,current=drafts[a.id]??a.substitute_plan??"",hasHandover=!!(a.handover_done||a.handover_not_done||a.handover_note);return <article key={a.id} style={{padding:"14px 0",borderTop:"1px solid #eee"}}>
    ...
    {ownAbsence?<div style={{marginTop:10}}><label style={{display:"block",fontSize:12,fontWeight:800,marginBottom:5}}>Vikarplan til denne time</label><textarea value={current} onChange={ev=>setDrafts(v=>({...v,[a.id]:ev.target.value}))} placeholder="Skriv hvad vikaren skal lave med klassen…" style={textarea}/><div style={{display:"flex",gap:7,flexWrap:"wrap",marginTop:7}}><button onClick={()=>savePlan(a)} disabled={saving===a.id} style={saveButton}>{saving===a.id?"Gemmer…":a.substitute_plan?"Gem ændringer":"Gem vikarplan"}</button>{(a.substitute_plan||current.trim())&&<button onClick={()=>clearPlan(a)} disabled={saving===a.id} style={clearButton}>Ryd vikarplan</button>}</div></div>:<div style={{marginTop:8,padding:10,borderRadius:8,background:a.substitute_plan?"#e7eee9":"#f3eee5",fontSize:13}}><strong>Vikarplan:</strong> {a.substitute_plan||"Mangler"}</div>}

    {ownAbsence&&hasHandover&&<section style={{marginTop:12,padding:"13px 14px",borderRadius:10,background:"#eef2ed",border:"1px solid #d7e0d8"}}><div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"center",flexWrap:"wrap"}}><small style={eyebrow}>VIKARENS OVERLEVERING</small>{a.handover_updated_at&&<small style={{color:"#6c756f",fontWeight:700}}>Opdateret {new Date(a.handover_updated_at).toLocaleString("da-DK",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"})}</small>}</div>{a.handover_done&&<div style={handoverRow}><strong>Det nåede de</strong><p>{a.handover_done}</p></div>}{a.handover_not_done&&<div style={handoverRow}><strong>Ikke nået</strong><p>{a.handover_not_done}</p></div>}{a.handover_note&&<div style={handoverRow}><strong>Vigtigt til dig</strong><p>{a.handover_note}</p></div>}</section>}
   </article>}):<p style={{color:"#707670"}}>Ingen vikardækninger denne dag.</p>}</section>
  </div>
 </section></main>
}
const card:React.CSSProperties={background:"white",border:"1px solid #ddd9d0",borderRadius:15,padding:22};
const textarea:React.CSSProperties={width:"100%",minHeight:82,padding:10,border:"1px solid #cfc9bd",borderRadius:8,boxSizing:"border-box",font:"inherit"};
const saveButton:React.CSSProperties={padding:"8px 12px",border:0,borderRadius:7,background:"#365044",color:"white",fontWeight:800,cursor:"pointer"};
const clearButton:React.CSSProperties={padding:"8px 12px",border:"1px solid #d7c4bf",borderRadius:7,background:"white",color:"#8a453b",fontWeight:800,cursor:"pointer"};
const eyebrow:React.CSSProperties={fontSize:10,fontWeight:900,letterSpacing:1.2,color:"#718077"};
const handoverRow:React.CSSProperties={marginTop:10,paddingTop:9,borderTop:"1px solid #dbe2dc",fontSize:13,lineHeight:1.5};
