...
export default function DanishWorkspace(){
 ...
 const grammarHref=room?`/grammar?mode=assign&class=${classId}&room=${room.id}`:`/grammar?mode=assign&class=${classId}`;
 const readingHref=room?`/grammar/laeseproeve?class=${classId}&room=${room.id}`:`/grammar/laeseproeve?class=${classId}`;
 const spellingHref=room?`/grammar/retskrivningsproeve?class=${classId}&room=${room.id}`:`/grammar/retskrivningsproeve?class=${classId}`;
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",color:"#26342e"}}>
  ...
  <section style={{maxWidth:1140,margin:"auto",padding:"26px 24px 80px"}}>
   ...
   <section style={{...card,display:"flex",justifyContent:"space-between",gap:14,alignItems:"end",flexWrap:"wrap"}}><div style={{display:"flex",gap:10,alignItems:"end",flexWrap:"wrap"}}><label style={label}>Klasse<select value={classId} onChange={e=>{const id=e.target.value?Number(e.target.value):"";setClassId(id);if(id)window.history.replaceState({},"",`/danish?class=${id}`)}} style={input}>{classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label style={label}>Se progression for<select value={grade} onChange={e=>setGrade(e.target.value?Number(e.target.value):"")} style={input}><option value="">Alle klassetrin</option>{Array.from({length:10},(_,i)=>i+1).map(n=><option key={n} value={n}>{n}. klasse</option>)}</select></label></div><div><small style={{display:"block",color:"#737a74",marginBottom:5}}>{selectedGrade==null?"Hele progressionen":danishGradeBand(selectedGrade)}</small><div style={{display:"flex",gap:7,flexWrap:"wrap"}}>{room&&<Link href={`/students/subjects/${room.id}`} style={secondaryLink}>← Dansk-fagrummet</Link>}{classId&&<Link href={`/students/class-learning-profile?class=${classId}`} style={secondaryLink}>Fagligt overblik →</Link>}</div></div></section>

   <section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(230px,1fr))",gap:12,marginTop:14}}>
    <ActionCard title="Skriveopgave" text={room?`Byg en genreopgave til ${selectedClass?.name||"klassen"} med elevens skrivehjælp tilpasset klassetrin.`:"Dansk-fagrummet skal være oprettet, før opgaven kan kobles sikkert til faget."} href={writingHref} action={room?"+ Ny dansk-opgave":"Åbn fagrum"}/>
    <ActionCard title="Tekstarbejde & analyse" text={room?"Tildel tekstnært arbejde med fx personer, miljø, fortæller, komposition, virkemidler, argumentation eller fortolkning. Stilladset tilpasses den enkelte elev.":"Dansk-fagrummet skal være oprettet, før tekstarbejdet kan tildeles sikkert."} href={analysisHref} action={room?"+ Nyt tekstarbejde":"Åbn fagrum"}/>
    <ActionCard title="Mundtlighed & kommunikation" text={room?"Planlæg fortælling, faglig forklaring, fremlæggelse, samtale, respons, debat, podcast, video eller prøveformidling med klassetrinstilpasset stikordsstøtte.":"Dansk-fagrummet skal være oprettet, før kommunikationsopgaven kan tildeles sikkert."} href={communicationHref} action={room?"+ Ny kommunikationsopgave":"Åbn fagrum"}/>
    <ActionCard title="Grammatik & sprog" text="Tildel målrettet grammatik, retskrivning, tegnsætning og tekstgrammatik med klassetrinsprogression." href={grammarHref} action="Åbn træning"/>
    <ActionCard title="Læseprøve & strategier" text="Tildel niveaudelt læseprøve 6.–9. klasse og brug strategiprofilen til målrettet opfølgning." href={readingHref} action="Åbn læsning"/>
    <ActionCard title="Retskrivningsprøve" text="Træn de prøve-lignende retskrivningsformer med eller uden tid på valgt niveau." href={spellingHref} action="Åbn retskrivning"/>
   </section>
...
const buildTag:React.CSSProperties={...liveTag,background:"#f1ece0",color:"#806b43",whiteSpace:"nowrap"};