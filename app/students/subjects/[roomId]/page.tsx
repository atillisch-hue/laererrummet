...
export default function SubjectRoom(){
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",color:"#26342e"}}>
  ...
  <section style={{maxWidth:1050,margin:"auto",padding:"30px 24px 80px"}}>
   ...
   {canEdit&&<section style={{...card,marginBottom:16,background:"#eef2ed"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}><div><small style={{fontWeight:900,color:"#718077"}}>FAGLÆRER</small><strong style={{display:"block",fontFamily:"Georgia,serif",fontSize:21,marginTop:3}}>Indret faglokalet</strong></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button onClick={()=>setEditingIntro(v=>!v)} style={secondary}>{editingIntro?"Luk":"Navn & intro"}</button><button onClick={()=>setShowAdd(v=>!v)} style={primary}>{showAdd?"Luk":"+ Tilføj indhold"}</button><Link href="/create-assignment" style={primaryLink}>+ Ny opgave</Link></div></div>
