...
export default function SubjectRoom(){
 ...
 const removeRoom=async()=>{
  ...
  const ok=window.confirm("Slet faglokalet permanent? Faglokalets opslag og materialer slettes. Opgaver og skemabrikker bevares, men mister koblingen til faglokalet.");if(!ok)return;
 ...
 const removeItem=async(id:number)=>{if(!canEdit||!window.confirm("Slet dette indhold fra faglokalet?"))return;const{error}=await supabase.from("subject_room_items").delete().eq("id",id);if(error)setMessage(error.message);else{if(editingItemId===id)cancelEditItem();await load()}};
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",color:"#26342e"}}>
  ...
  <section style={{maxWidth:1050,margin:"auto",padding:"30px 24px 80px"}}>
   ...
   {canEdit&&<section style={{...card,marginBottom:16,background:"#eef2ed"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}><div><small style={{fontWeight:900,color:"#718077"}}>FAGLÆRER</small><strong style={{display:"block",fontFamily:"Georgia,serif",fontSize:21,marginTop:3}}>Indret faglokalet</strong></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button onClick={()=>setEditingIntro(v=>!v)} style={secondary}>{editingIntro?"Luk":"Redigér navn & intro"}</button><button onClick={()=>setShowAdd(v=>!v)} style={primary}>{showAdd?"Luk":"+ Tilføj indhold"}</button><Link href={createAssignmentHref} style={primaryLink}>+ Ny opgave</Link><button onClick={removeRoom} disabled={saving} style={danger}>Slet faglokale</button></div></div>
