...
type ScheduleOccurrence={schedule_entry_id:number;class_id:number;subject:string;start_time:string;end_time:string;room:string|null;entry_kind:EntryKind};
...
export default function StudentsPage(){
 ...
 async function loadSchedule(cid:number,d:string){
  const{data,error}=await supabase.rpc("staff_class_schedule_occurrences",{p_class_id:cid,p_start_date:d,p_end_date:d});
  if(error){setSchedule([]);setMsg("Dagens skema kunne ikke hentes for denne klasse.");return}
  setSchedule(((data||[]) as any[]).map(row=>({schedule_entry_id:Number(row.schedule_entry_id),class_id:Number(row.class_id),subject:String(row.subject||""),start_time:String(row.start_time||""),end_time:String(row.end_time||""),room:row.room?String(row.room):null,entry_kind:String(row.entry_kind||"other") as EntryKind})).sort((a,b)=>a.start_time.localeCompare(b.start_time)));
 }
 ...
 async function load(){
  ...
  const[cRes,sRes,tcRes,aRes,hRes,saRes]=await Promise.all([
   ...
   supabase.from("substitute_assignments").select("assignment_date,schedule_entry_id").eq("substitute_teacher_id",u.id).eq("assignment_date",d)
  ]);
  const assigned=new Set((tcRes.data||[]).map((x:any)=>Number(x.class_id)));
  const subIds=(saRes.data||[]).map((x:any)=>Number(x.schedule_entry_id)).filter(Boolean);
  if(subIds.length){
   const subEntries=await supabase.from("schedule_entries").select("id,class_id").in("id",subIds);
   for(const entry of subEntries.data||[])assigned.add(Number(entry.class_id));
  }
  const all=(cRes.data||[]) as ClassRow[],cs=isAdmin?all:all.filter(x=>assigned.has(x.id));
  setClasses(cs);
  setStudents(((sRes.data||[]) as Student[]).filter(x=>isAdmin||assigned.has(x.class_id)));
  setRows((aRes.data||[]) as Absence[]);setHandovers((hRes.data||[]) as Handover[]);
  const requested=Number(p.get("class"));const nextClass=cs.some(x=>x.id===requested)?requested:(cs[0]?.id||"");setClassId(nextClass);
  if(nextClass!=="")await loadSchedule(Number(nextClass),d);else setSchedule([]);
  const problem=cRes.error||sRes.error||tcRes.error||aRes.error||hRes.error||saRes.error;if(problem)setMsg(problem.message);
  setReady(true);
 }
 useEffect(()=>{load()},[]);
 ...
 async function changeClass(next:number){setClassId(next);setTab("today");setEditingAbsenceId(null);await loadSchedule(next,date)}
 async function changeDate(next:string){setDate(next);setEditingAbsenceId(null);if(classId!=="")await loadSchedule(Number(classId),next)}
