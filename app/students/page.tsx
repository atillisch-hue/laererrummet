...
type Absence={id:number;student_id:number;absence_date:string;status:string;note:string|null;source?:string};
type ScheduleEntry={id:number;class_id:number;subject:string;weekday:number;start_time:string;end_time:string;room:string|null};
...
export default function StudentsPage(){
 ...
 async function load(){
  ...
  const[{data:c},{data:s},{data:tc},{data:a},{data:se},{data:h},{data:sa}]=await Promise.all([
   ...
   supabase.from("student_absence").select("id,student_id,absence_date,status,note,source").order("absence_date",{ascending:false}).limit(300),
   supabase.from("schedule_entries").select("id,class_id,subject,weekday,start_time,end_time,room"),
  ...
  const entries=(se||[]) as ScheduleEntry[],assigned=new Set((tc||[]).map((x:any)=>Number(x.class_id))),subIds=new Set((sa||[]).filter((x:any)=>x.assignment_date===d).map((x:any)=>Number(x.schedule_entry_id)));
  entries.filter(x=>subIds.has(x.id)).forEach(x=>assigned.add(x.class_id));
  const all=(c||[]) as ClassRow[],cs=hasRole(u,"admin")?all:all.filter(x=>assigned.has(x.id));
  setClasses(cs);setStudents(((s||[]) as Student[]).filter(x=>hasRole(u,"admin")||assigned.has(x.class_id)));setRows((a||[]) as Absence[]);setSchedule(entries);setHandovers((h||[]) as Handover[]);
 ...
 const rowFor=(id:number)=>todayRows.find(r=>r.student_id===id),studentName=(id:number|null)=>id?students.find(s=>s.id===id)?.name||"Elev":"Hele klassen",label=(s:string)=>TYPES.find(x=>x.value===s)?.label||s,hlabel=(s:string)=>HT.find(x=>x.value===s)?.label||s;

 async function save(){
  const chosen=shown.filter(s=>selected[s.id]);if(!chosen.length){setMsg("Markér mindst én elev med fravær.");return}
  const{error}=await supabase.from("student_absence").insert(chosen.map(s=>({student_id:s.id,absence_date:date,status:types[s.id]||"sick",note:(notes[s.id]||"").trim()||null,source:"teacher"})));
  setMsg(error?error.message:`${chosen.length} registrering${chosen.length===1?"":"er"} gemt.`);if(!error){setSelected({});setNotes({});await load()}
 }
 async function addHandover(){if(!hm.trim()||!classId)return;const{error}=await supabase.from("class_handover").insert({class_id:classId,student_id:hs||null,handover_date:date,category:ht,message:hm.trim(),author_id:userId,author_email:email});if(!error){setHm("");setHs("");await load()}}
 async function removeHandover(id:number){await supabase.from("class_handover").delete().eq("id",id).eq("author_id",userId);await load()}

 const HandoverList=({items}:{items:Handover[]})=><div style={{display:"grid",gap:10}}>{items.length?items.map(h=><article key={h.id} style={{background:"#faf9f6",border:"1px solid #e0ddd5",borderRadius:11,padding:"14px 16px",position:"relative"}}>{h.author_id===userId&&<button onClick={()=>removeHandover(h.id)} style={{position:"absolute",right:8,top:6,border:0,background:"transparent",fontSize:18,cursor:"pointer"}}>×</button>}<div><span style={{fontSize:11,fontWeight:900,background:"#e7eee9",padding:"4px 7px",borderRadius:999}}>{hlabel(h.category)}</span> <strong style={{fontSize:13}}>{studentName(h.student_id)}</strong></div><p style={{margin:"9px 20px 7px 0"}}>{h.message}</p><small style={{color:"#777"}}>{h.author_email.split("@")[0]} · {new Date(h.created_at).toLocaleString("da-DK")}</small></article>):<p style={{color:"#707670"}}>Ingen overleveringer endnu.</p>}</div>;
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",color:"#26342e"}}>
  ...
  <section style={{maxWidth:1100,margin:"auto",padding:"28px 24px 70px"}}>
   ...
   {tab==="today"&&<div style={{display:"grid",gap:18}}><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:16}}><section style={card}><small style={{fontWeight:900,color:"#718077"}}>OVERLEVERING</small><h2 style={{fontFamily:"Georgia,serif",margin:"4px 0 12px"}}>Det skal du vide</h2><HandoverList items={todayHandovers.slice(0,3)}/>{todayHandovers.length>3&&<button onClick={()=>setTab("handover")} style={{marginTop:10,border:0,background:"transparent",fontWeight:800,color:"#526b60"}}>Se alle →</button>}</section><section style={card}><small style={{fontWeight:900,color:"#718077"}}>DAGENS SKEMA</small><h2 style={{fontFamily:"Georgia,serif",margin:"4px 0 12px"}}>I dag</h2>{daySchedule.length?<div style={{display:"grid",gap:7}}>{daySchedule.map(l=><Link key={l.id} href={`/calendar/lesson/${l.id}?date=${date}`} style={{display:"grid",gridTemplateColumns:"78px 1fr",gap:10,padding:"10px",border:"1px solid #e4e1da",borderRadius:9,background:"#faf9f6",textDecoration:"none",color:"inherit"}}><strong>{l.start_time.slice(0,5)}</strong><span><strong>{l.subject}</strong>{l.room&&<small style={{display:"block"}}>Lokale {l.room}</small>}</span></Link>)}</div>:<p>Ingen undervisning på skemaet.</p>}</section></div>
    <section style={card}><div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:12}}><div><small style={{fontWeight:900,color:"#718077"}}>FRAVÆR</small><h2 style={{fontFamily:"Georgia,serif",fontSize:29,margin:"5px 0"}}>Hvem er her?</h2><p style={{color:"#707670"}}>Alle regnes som til stede. Registrér kun fravær eller forsinkelse.</p></div><input type="date" value={date} onChange={e=>setDate(e.target.value)} style={{height:40,padding:"0 10px"}}/></div>{msg&&<div style={{padding:11,background:"#e7eee9",borderRadius:8}}>{msg}</div>}{shown.map(s=>{const ex=rowFor(s.id),parent=ex?.source==="parent";return <div key={s.id} style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10,alignItems:"center",borderTop:"1px solid #eee",padding:"11px 0"}}><label style={{display:"flex",gap:8,alignItems:"center",fontWeight:700}}><input type="checkbox" disabled={!!ex} checked={!!selected[s.id]} onChange={e=>setSelected(v=>({...v,[s.id]:e.target.checked}))}/><span>{s.name}{ex&&<small style={{display:"block",color:parent?"#775f2e":"#687068"}}>{label(ex.status)}{parent?" · meldt af forælder":""}</small>}</span></label><select disabled={!!ex||!selected[s.id]} value={types[s.id]||"sick"} onChange={e=>setTypes(v=>({...v,[s.id]:e.target.value}))} style={{padding:8}}>{TYPES.map(t=><option key={t.value} value={t.value}>{t.label}</option>)}</select>{ex?<div style={{fontSize:13,color:"#687068"}}>{ex.note||"Registreret"}</div>:<input disabled={!selected[s.id]} value={notes[s.id]||""} onChange={e=>setNotes(v=>({...v,[s.id]:e.target.value}))} placeholder="Note (valgfri)" style={{padding:8}}/>}</div>})}<button onClick={save} style={{marginTop:16,padding:"10px 15px",border:0,borderRadius:8,background:"#365044",color:"white",fontWeight:900}}>Gem ændringer</button></section></div>}
