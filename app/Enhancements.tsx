...
export default function Enhancements(){
  ...
  useEffect(()=>{
    ...
    const addFeedback=async(writing:Element)=>{
      ...
      feedbackLoading=true;const{data,error}=await supabase.rpc("student_feedback",{p_access_code:code});feedbackLoading=false;if(error)return;
      const items=Array.isArray(data?.feedback)?data.feedback:[];if(!items.length)return;
      const heading=document.querySelector(".studentContent h1")?.textContent?.trim()||"";if(!heading)return;
      let item=items.length===1?items[0]:null;if(!item?.text?.trim())return;
      ...
      const text=document.createElement("p");text.textContent=item.text;text.style.cssText="white-space:pre-wrap;line-height:1.6;margin:0;color:#4f554f";card.append(label,title,text);writing.appendChild(card);
    };
    ...
    const enhance=()=>{
      ...
      if(writing&&!document.getElementById("manual-save-wrap")){
        const oldStatus=writing.querySelector(".saved");const wrap=document.createElement("div");wrap.id="manual-save-wrap";wrap.style.cssText="display:flex;align-items:center;gap:12px;justify-content:space-between;margin-top:18px;flex-wrap:wrap";
        ...
        const actions=document.createElement("div");actions.style.cssText="display:flex;align-items:center;gap:12px;margin-left:auto";const status=document.createElement("span");status.id="manual-save-status";status.textContent="Alt er gemt ✓";status.style.cssText="font-size:14px;color:#55705d;font-weight:700";
        ...
        writing.addEventListener("input",()=>{status.textContent="Gemmer…";updateProgress();window.setTimeout(()=>{if(status.textContent==="Gemmer…")status.textContent="Alt er gemt ✓"},700)});if(oldStatus)oldStatus.remove();actions.append(status,button);wrap.append(progress,actions);writing.appendChild(wrap);updateProgress();
      }
      ...
      const assignmentRows=Array.from(document.querySelectorAll("main.shell .assignments .assignment")) as HTMLElement[];
      assignmentRows.forEach(row=>{
        if(row.querySelector(".delete-assignment-button"))return;
        const title=(row.querySelector("strong")?.textContent||"").trim();if(!title)return;
        const del=document.createElement("button");del.type="button";del.className="delete-assignment-button";del.textContent="Slet";del.style.cssText="margin-left:10px;border:1px solid #d9b9b4;border-radius:7px;background:#fff7f5;color:#9b3b32;padding:6px 9px;font-weight:700;cursor:pointer;align-self:center";
        del.onclick=async()=>{const ok=window.confirm(`Slet opgaven \"${title}\"?\n\nAlle elevers besvarelser og lærerfeedback til denne opgave slettes permanent. Handlingen kan ikke fortrydes.`);if(!ok)return;del.disabled=true;del.textContent="Sletter…";
          const className=(document.querySelector("main.shell section.content header h1")?.textContent||"").trim();const{data:cls}=await supabase.from("classes").select("id").eq("name",className).limit(1).maybeSingle();if(!cls){alert("Kunne ikke finde klassen.");del.disabled=false;del.textContent="Slet";return}
          const{data:matches,error:findError}=await supabase.from("assignments").select("id,title").eq("class_id",cls.id).eq("title",title);if(findError||!matches?.length){alert("Kunne ikke finde opgaven.");del.disabled=false;del.textContent="Slet";return}if(matches.length>1){alert("Der findes flere opgaver med samme titel i klassen. Sletning er stoppet af sikkerhedshensyn.");del.disabled=false;del.textContent="Slet";return}const id=matches[0].id;
          const{error:feedbackError}=await supabase.from("feedback").delete().eq("assignment_id",id);if(feedbackError){alert("Kunne ikke slette feedback: "+feedbackError.message);del.disabled=false;del.textContent="Slet";return}
          const{error:draftError}=await supabase.from("drafts").delete().eq("assignment_id",id);if(draftError){alert("Kunne ikke slette elevbesvarelser: "+draftError.message);del.disabled=false;del.textContent="Slet";return}
          const{error:assignmentError}=await supabase.from("assignments").delete().eq("id",id);if(assignmentError){alert("Kunne ikke slette opgaven: "+assignmentError.message);del.disabled=false;del.textContent="Slet";return}window.location.reload()};row.appendChild(del);
      });

      const studentRows=Array.from(document.querySelectorAll("main.shell .students .student")) as HTMLElement[];
      studentRows.forEach(row=>{if(row.querySelector(".delete-student-button"))return;const name=(row.querySelector("b")?.textContent||"").trim();if(!name)return;const del=document.createElement("button");del.type="button";del.className="delete-student-button";del.textContent="Slet";del.title=`Slet ${name}`;del.style.cssText="border:1px solid #d9b9b4;border-radius:7px;background:#fff7f5;color:#9b3b32;padding:7px 9px;font-weight:700;cursor:pointer";del.onclick=async()=>{const ok=window.confirm(`Slet ${name}?\n\nElevens gemte arbejde bliver også slettet permanent. Denne handling kan ikke fortrydes.`);if(!ok)return;del.disabled=true;del.textContent="Sletter…";const className=(document.querySelector("main.shell section.content header h1")?.textContent||"").trim();const{data:cls,error:classError}=await supabase.from("classes").select("id").eq("name",className).limit(1).maybeSingle();if(classError||!cls){alert("Kunne ikke finde elevens klasse. Prøv igen.");del.disabled=false;del.textContent="Slet";return}const{data:matches,error:studentError}=await supabase.from("students").select("id,name").eq("class_id",cls.id).eq("name",name);if(studentError||!matches?.length){alert("Kunne ikke finde eleven i databasen.");del.disabled=false;del.textContent="Slet";return}if(matches.length>1){alert("Der er flere elever med samme navn i klassen. Sletning er stoppet af sikkerhedshensyn.");del.disabled=false;del.textContent="Slet";return}const studentId=matches[0].id;await supabase.from("feedback").delete().eq("student_id",studentId);const{error:draftError}=await supabase.from("drafts").delete().eq("student_id",studentId);if(draftError){alert("Kunne ikke slette elevens besvarelser: "+draftError.message);del.disabled=false;del.textContent="Slet";return}const{error:deleteError}=await supabase.from("students").delete().eq("id",studentId);if(deleteError){alert("Kunne ikke slette eleven: "+deleteError.message);del.disabled=false;del.textContent="Slet";return}window.location.reload()};row.appendChild(del)});
    };
