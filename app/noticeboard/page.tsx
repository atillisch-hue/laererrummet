...
export default function Noticeboard(){
 ...
 useEffect(()=>{(async()=>{
  ...
  const[eRes,stRes,cRes,pRes,aRes,mRes]=await Promise.all([
 ...
 const chooseFiles=(files:FileList|null)=>{
  const picked=Array.from(files||[]).slice(0,5);const invalid=picked.find(f=>f.size>20*1024*1024||!allowedExtensions.has(extension(f.name)));
  if(invalid){setMessage(`Filen ${invalid.name} kan ikke vedhæftes. Brug Word, PDF, Office, tekst eller et billede på højst 20 MB.`);return}setMessage("");setSelectedFiles(picked);
 };
 const add=async()=>{
  if((!text.trim()&&!selectedFiles.length)||!userId||audiences.length===0||busy)return;
  ...
  for(const file of selectedFiles){
   const ext=extension(file.name),mime=file.type||mimeByExtension[ext],path=`school-${post.school_id}/noticeboard/${post.id}/${crypto.randomUUID()}-${safeFileName(file.name)}`;
   const{error:metaError}=await supabase.from("school_files").insert({school_id:post.school_id,area:"noticeboard",object_path:path,display_name:file.name,mime_type:mime,size_bytes:file.size,created_by:userId,noticeboard_post_id:post.id});
   if(metaError){if(uploaded.length)await supabase.storage.from("school-files").remove(uploaded);await supabase.from("noticeboard_posts").delete().eq("id",post.id);setMessage(`Opslaget blev ikke oprettet, fordi ${file.name} ikke kunne registreres sikkert.`);setBusy(false);return}
   const{error:uploadError}=await supabase.storage.from("school-files").upload(path,file,{contentType:mime,upsert:false});
   if(uploadError){await supabase.from("school_files").delete().eq("object_path",path);if(uploaded.length)await supabase.storage.from("school-files").remove(uploaded);await supabase.from("noticeboard_posts").delete().eq("id",post.id);setMessage(`Upload af ${file.name} mislykkedes. Ingen dele af opslaget blev gemt.`);setBusy(false);return}
   uploaded.push(path);
  }
 ...
 const weekDays=useMemo(()=>{const monday=mondayFor(selectedDate);return Array.from({length:5},(_,i)=>{const d=new Date(monday);d.setDate(d.getDate()+i);return d})},[selectedDate]);
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",color:"#26342e"}}>
  ...
  <section style={{maxWidth:1120,margin:"auto",padding:"28px 26px 60px"}}>
   <section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:12,marginBottom:18}}>
    <article style={overviewCard}><div style={{display:"flex",justifyContent:"space-between",gap:9,alignItems:"start"}}><div><p style={eyebrow}>{isToday?"I DAG · ":""}{selectedLabel.toUpperCase()}</p><h2 style={overviewTitle}>Min dag</h2></div><div style={{display:"flex",gap:5}}><button onClick={()=>moveDay(-1)} style={arrowButton}>←</button>{!isToday&&<button onClick={()=>setSelectedDate(iso(new Date()))} style={arrowButton}>•</button>}<button onClick={()=>moveDay(1)} style={arrowButton}>→</button></div></div>{selectedItems.length===0?<div style={okBox}>✓ Ingen ændringer i dit skema</div>:<div style={{display:"grid",gap:7,marginTop:10}}>{selectedItems.slice(0,3).map((item,i)=>{const x=item.entry,a=item.assignment;return <div key={`${item.substitute?"v":"f"}-${x.id}-${i}`} style={{padding:"8px 9px",borderRadius:8,background:item.substitute?"#e7eee9":"#f3e8d8"}}><strong style={{fontSize:12}}>{item.substitute?"VIKAR":"FRAVÆR"} · {x.start_time.slice(0,5)}–{x.end_time.slice(0,5)}</strong><div style={{fontWeight:850,marginTop:2}}>{x.subject} · {className(x.class_id)}</div>{item.substitute&&<small>for {teacherName(item.absentId)}</small>}{item.substitute&&a?.substitute_plan&&<div style={{fontSize:12,marginTop:4}}>Vikarplan: {a.substitute_plan}</div>}{!item.substitute&&a&&<details style={{marginTop:5}}><summary style={{cursor:"pointer",fontSize:12,fontWeight:800}}>Tilføj vikarplan</summary><textarea value={planDrafts[a.id]??a.substitute_plan??""} onChange={e=>setPlanDrafts(v=>({...v,[a.id]:e.target.value}))} placeholder="Vikarplan…" style={{width:"100%",boxSizing:"border-box",minHeight:55,marginTop:6}}/><button onClick={()=>savePlan(a)} disabled={savingPlan===a.id}>{savingPlan===a.id?"Gemmer…":"Gem"}</button></details>}</div>})}{selectedItems.length>3&&<small>+ {selectedItems.length-3} flere ændringer</small>}</div>}<Link href={`/calendar?date=${selectedDate}`} style={cardLink}>Åbn dagen i Kalender →</Link></article>

    <article style={overviewCard}><p style={eyebrow}>KALENDER</p><h2 style={overviewTitle}>Denne uge</h2><div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:5,marginTop:11}}>{weekDays.map(d=>{const s=iso(d),active=s===selectedDate,count=dayLoad(d);return <button key={s} onClick={()=>setSelectedDate(s)} style={{border:active?"2px solid #486b59":"1px solid #ddd9d0",background:active?"#e7eee9":"#faf9f6",borderRadius:9,padding:"8px 3px",cursor:"pointer",color:"#26342e"}}><small style={{display:"block",fontWeight:900,textTransform:"uppercase",fontSize:9}}>{d.toLocaleDateString("da-DK",{weekday:"short"}).replace(".","")}</small><strong style={{display:"block",fontFamily:"Georgia,serif",fontSize:20,margin:"2px 0"}}>{d.getDate()}</strong><span style={{fontSize:9,color:"#69716c"}}>{count?`${count} punkt${count===1?"":"er"}`:"ro"}</span></button>})}</div><Link href={`/calendar?date=${selectedDate}`} style={cardLink}>Se hele ugen →</Link></article>
