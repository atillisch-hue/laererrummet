...
type CalendarEvent={date:string;label:string;type:string};
type ClosedDay={date:string;label:string};
type WeekMark={key:string;type:string;label:string;dates:string[]};
...
function eventTypeLabel(type:string){return({pedagogical:"Pædagogisk dag",special_week:"Specialuge",project_week:"Projekt-/faguge",trip:"Lejrskole/tur",event:"Arrangement",other:"Andet",closed:"Lukket"} as Record<string,string>)[type]||"Skolekalender"}
...
export default function Noticeboard(){
 ...
 const schoolMark=(date:string)=>{const closed=closedDays.find(x=>x.date===date);if(closed)return{date,label:closed.label||"Lukket",type:"closed"};return calendarEvents.find(x=>x.date===date)||null};
 const weekMarks=useMemo(()=>{const grouped=new Map<string,WeekMark>();for(const d of weekDays){const date=iso(d),mark=schoolMark(date);if(!mark)continue;const key=`${mark.type}:${mark.label}`,current=grouped.get(key)||{key,type:mark.type,label:mark.label,dates:[]};current.dates.push(date);grouped.set(key,current)}return [...grouped.values()]},[weekDays,calendarEvents,closedDays]);
