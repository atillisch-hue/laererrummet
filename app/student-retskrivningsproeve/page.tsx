...
import {buildSpellingExamSet,spellingExamSectionCounts,type SpellingExamQuestion} from "../student-grammar/fp9-spelling-exam-bank";
...
type Assignment={id:number;title:string;time_limit_minutes:number|null;question_count:number;submitted?:boolean;score?:number|null;max_score?:number|null};
...
export default function StudentSpellingExam(){
 ...
 useEffect(()=>{(async()=>{
  ...
  try{setQuestions(buildSpellingExamSet(Number(start.question_seed),Number(start.question_count||30)))}catch{setError("Prøvesættet kunne ikke bygges. Kontakt din lærer.");setLoading(false);return}
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",padding:"28px 20px 70px",color:"#26342e"}}><section style={{maxWidth:900,margin:"auto"}}>
  ...
  {error&&!assignment?<div style={card}><h1>Hov</h1><p>{error}</p></div>:<>
   <div style={{marginTop:28,display:"flex",justifyContent:"space-between",alignItems:"start",gap:16,flexWrap:"wrap"}}><div><p style={eyebrow}>UDSKOLING · RETSKRIVNING</p><h1 style={{fontFamily:"Georgia,serif",fontSize:40,margin:"6px 0 8px"}}>{assignment?.title||"Træn retskrivningsprøven"}</h1><p style={{color:"#6b736e",fontSize:17,lineHeight:1.5,maxWidth:650}}>Prøvelignende træning med stavning, bøjning, sprogbrug, korrektur og tegnsætning. Opgaverne er Klasseværelsets egne og er ikke en officiel prøve.</p></div>
