...
import {READING_STRATEGY_COACHING,weakestReadingStrategy} from "../student-reading-exam/strategy-feedback";
...
export default function StudentReadingExam(){
 ...
 const nextFocus=useMemo(()=>weakestReadingStrategy(strategyResults as any),[strategyResults]);
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",padding:"28px 20px 70px",color:"#26342e"}}><section style={{maxWidth:1050,margin:"auto"}}>
  ...
  {error&&!assignment?<div style={card}><h1>Hov</h1><p>{error}</p></div>:<>
   ...
   {submitted?<div style={{...card,marginTop:24}}><p style={eyebrow}>{timedOut?"TIDEN UDLØB · AUTOMATISK AFLEVERET":"AFLEVERET"}</p><h2 style={{fontFamily:"Georgia,serif",fontSize:34,margin:"6px 0"}}>{score??assignment?.score??0} / {total||assignment?.max_score||assignment?.question_count||level.questionCount}</h2>{elapsed!==null&&<p>Tidsforbrug: <strong>{formatTime(elapsed)}</strong></p>}{total>0&&<><h3 style={{fontFamily:"Georgia,serif",marginTop:24}}>Dine læsestrategier</h3><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:8}}>{Object.entries(strategyResults).filter(([,row])=>row.total>0).map(([strategy,row])=><div key={strategy} style={resultBox}><strong>{strategy}</strong><span>{row.correct} / {row.total}</span></div>)}</div>{nextFocus?<div style={{marginTop:18,padding:18,borderRadius:12,background:"#e9f0eb",border:"1px solid #cbd9cf"}}><p style={eyebrow}>DIT NÆSTE LÆSEFOKUS</p><h3 style={{fontFamily:"Georgia,serif",fontSize:25,margin:"5px 0"}}>{nextFocus.strategy}</h3><p style={{margin:"4px 0",lineHeight:1.5}}>Du havde <strong>{nextFocus.correct}/{nextFocus.total}</strong> i denne strategi. {READING_STRATEGY_COACHING[nextFocus.strategy].explanation}</p><p style={{margin:"8px 0 0",lineHeight:1.5}}><strong>Strategi:</strong> {READING_STRATEGY_COACHING[nextFocus.strategy].move}</p><button onClick={()=>window.location.href=`/student-reading-training?strategy=${encodeURIComponent(nextFocus.strategy)}&grade=${grade}&from=${assignment?.id||""}`} style={{...primary,marginTop:13}}>Træn {nextFocus.strategy.toLowerCase()} →</button></div>:<div style={{marginTop:18,padding:17,borderRadius:12,background:"#edf5ef",border:"1px solid #d0dfd4"}}><strong>Alle målte læsestrategier er sikre ✓</strong><p style={{margin:"5px 0 0",color:"#5d6962"}}>Du havde ingen fejl i strategiprofilen på dette sæt. Du kan stadig vælge en strategi og træne videre.</p><button onClick={()=>window.location.href=`/student-reading-training?grade=${grade}&from=${assignment?.id||""}`} style={{...primary,marginTop:12}}>Vælg læsestrategi →</button></div>}<h3 style={{fontFamily:"Georgia,serif",marginTop:24}}>Teksttyper</h3><div style={{display:"flex",gap:7,flexWrap:"wrap"}}>{Object.entries(sectionResults).map(([section,row])=><span key={section} style={chip}>{section}: {row.correct}/{row.total}</span>)}</div><div style={{display:"grid",gap:12,marginTop:24}}>{parts.map(part=><details key={part.id} style={{background:"#faf9f6",border:"1px solid #e2ded6",borderRadius:10,padding:12}}><summary style={{fontWeight:900,cursor:"pointer"}}>{part.title} · {sectionResults[part.section]?.correct||0}/{part.questions.length}</summary><div style={{display:"grid",gap:8,marginTop:12}}>{part.questions.map(question=>{const value=answers[key(part,question)]||"",ok=value===question.answer;return <div key={question.id} style={{padding:10,borderRadius:8,background:ok?"#edf5ef":"#fff3ef"}}><strong>{ok?"✓":"✕"} {question.q}</strong>{!ok&&<div style={{fontSize:13,marginTop:4}}>Dit svar: <strong>{value||"—"}</strong> · Korrekt: <strong>{question.answer}</strong></div>}<div style={{fontSize:12,color:"#66706a",marginTop:4}}>{question.strategy} · {question.explanation}</div></div>})}</div></details>)}</div></>}<button onClick={()=>window.location.href="/?student=1"} style={{...primary,marginTop:18}}>Til mine opgaver →</button></div>:
