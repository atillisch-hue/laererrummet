...
export default function StudentReadingExam(){
 ...
 useEffect(()=>{(async()=>{
  ...
  try{setParts(buildReadingExamSet(Number(start.question_seed),Number(start.target_grade||found.target_grade||9)))}catch{setError("Læseprøvesættet kunne ikke bygges. Kontakt din lærer.");setLoading(false);return}
 ...
 async function submitExam(fromTimer=false){
  ...
  const snapshot=Object.fromEntries(flatQuestions.map(({part,question},i)=>[i,{question:question.q,section:part.section,strategy:question.strategy,studentAnswer:answers[key(part,question)]||"",correctAnswer:question.answer,correct:answers[key(part,question)]===question.answer,explanation:question.explanation,textId:part.id}]));
