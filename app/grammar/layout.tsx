...
export default function GrammarLayout({children}:{children:ReactNode}){
 ...
 return <>
  <div style={{background:"#263f35",color:"white"}}><div style={{maxWidth:1100,margin:"auto",padding:"9px 24px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><strong style={{fontSize:12,marginRight:4}}>Grammatik & sprog</strong><Link href={`/grammar${roomSuffix}`} style={navLink}>Træningsopgaver</Link><Link href={`/grammar/retskrivningsproeve${roomSuffix}`} style={navLink}>Træn retskrivningsprøven</Link><Link href={`/grammar/laeseproeve${roomSuffix}`} style={navLink}>Træn læseprøven</Link></div></div>
