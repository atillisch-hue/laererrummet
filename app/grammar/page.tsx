...
import {grammarTopicMeta,liveGrammarAreas} from "./grammar-catalog";

const areas=liveGrammarAreas.map(area=>({...area,topics:area.topics.map(topic=>topic.title)}));
...
type AnswerDetail={question?:string;answer?:string;studentAnswer?:string;correctAnswer?:string;correct?:boolean;explanation?:string;round?:number};
type AttemptHistory={attempt_number:number;score:number;max_score:number;completed_at:string;answers?:Record<string,string|AnswerDetail>|null};
type Result={student_id:number;student_name:string;completed:boolean;score:number|null;max_score:number|null;completed_at:string|null;answers?:Record<string,string|AnswerDetail>|null;attempts?:number;best_score?:number|null;best_max_score?:number|null;mastered?:boolean;attempt_history?:AttemptHistory[]};
...
export default function GrammarPage(){
 ...
 const recipientStudents=useMemo(()=>recipientMode==="class"?classStudents:classStudents.filter(s=>selectedStudents.includes(s.id)),[recipientMode,classStudents,selectedStudents]);
 const missingGradeStudents=useMemo(()=>recipientStudents.filter(s=>s.grade_level===null||s.grade_level===undefined),[recipientStudents]);
 const topicMeta=useMemo(()=>grammarTopicMeta(topic),[topic]);
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",padding:"30px 24px 80px",color:"#26342e"}}><section style={{maxWidth:1080,margin:"0 auto"}}>
  ...
  <p className="eyebrow" style={{marginTop:28}}>GRAMMATIK & SPROG</p><h1 style={{marginBottom:8}}>Tildel målrettet sprogtræning</h1><p style={{fontSize:17,color:"#707670",maxWidth:820,lineHeight:1.5,marginTop:0}}>Vælg fagligt område, emne, niveau og hvem der skal have det. Opgaverne tilpasses elevens klassetrin, og Udfordring kan løfte eleven lidt over den normale progression.</p>

  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:12,marginTop:25}}>{areas.map((a,i)=><button key={a.title} onClick={()=>chooseArea(i)} style={{background:selected===i?"#edf1ec":"white",border:selected===i?"2px solid #526b60":"1px solid #dfdcd4",borderRadius:13,padding:19,textAlign:"left",cursor:"pointer",minHeight:155}}><span style={{fontFamily:"Georgia,serif",fontWeight:800,fontSize:18,color:"#365044"}}>{a.icon}</span><strong style={{display:"block",fontFamily:"Georgia,serif",fontSize:21,marginTop:12,color:"#27352d"}}>{a.title}</strong><span style={{display:"block",color:"#727772",lineHeight:1.4,marginTop:6,fontSize:14}}>{a.text}</span><small style={{display:"block",marginTop:10,color:"#526b60",fontWeight:900}}>{a.topics.length} aktive emner</small></button>)}</div>

  {selected!==null&&<div style={panel}><strong>1. Vælg emne</strong><div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:10}}>{areas[selected].topics.map(t=>{const meta=grammarTopicMeta(t);return <button key={t} onClick={()=>setTopic(t)} style={{...smallButton,border:topic===t?"2px solid #526b60":"1px solid #d8d5cd",background:topic===t?"#edf1ec":"#fff",display:"grid",gap:2,textAlign:"left"}}><span>{t}</span>{meta&&<small style={{fontSize:10,color:"#7a837d"}}>fra {meta.minGrade}. klasse</small>}</button>})}</div></div>}
  {topic&&<div style={panel}><strong>2. Vælg niveau</strong>{topicMeta&&<div style={{marginTop:9,padding:"9px 11px",background:"#f4f1ea",borderRadius:8,fontSize:13,color:"#59615c"}}><strong>{topic}</strong> er placeret fra <strong>{topicMeta.minGrade}. klasse</strong> i progressionen. Basis holder sig på trinnet; Udfordring kan åbne indhold cirka ét trin tidligere for stærke elever.</div>}<div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:10}}>{levels.map(l=><button key={l.id} onClick={()=>setLevel(l.id)} style={button(level===l.id)}>{l.title}</button>)}</div><div style={{marginTop:18,paddingTop:18,borderTop:"1px solid #ebe7de"}}><strong>3. Hvem skal have den?</strong><div style={{display:"flex",gap:10,alignItems:"end",flexWrap:"wrap",marginTop:10}}><label style={{fontWeight:800,fontSize:13}}>Klasse<select value={classId} onChange={e=>changeClass(Number(e.target.value))} style={{...field,minWidth:210}}>{classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><button onClick={()=>{setRecipientMode("class");setSelectedStudents([])}} style={button(recipientMode==="class")}>Hele klassen</button><button onClick={()=>setRecipientMode("students")} style={button(recipientMode==="students")}>Udvalgte elever</button></div>{recipientMode==="students"&&<div style={{marginTop:12}}>{classStudents.length?<div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{classStudents.map(s=><button key={s.id} onClick={()=>toggleStudent(s.id)} style={{...smallButton,border:selectedStudents.includes(s.id)?"2px solid #526b60":"1px solid #d8d5cd",borderRadius:999,background:selectedStudents.includes(s.id)?"#edf1ec":"white"}}>{selectedStudents.includes(s.id)?"✓ ":""}{s.name}{s.grade_level!==null&&s.grade_level!==undefined?` · ${s.grade_level}.`:" · ?"}</button>)}</div>:<p style={{margin:"8px 0 0",color:"#8a5d2d",fontWeight:700}}>Der er ingen elever registreret i denne klasse endnu.</p>}</div>}{missingGradeStudents.length>0&&<div style={{marginTop:14,padding:"11px 13px",borderRadius:9,background:"#fff7e8",border:"1px solid #ead8ad",color:"#665431",lineHeight:1.45}}><strong>Klassetrin mangler for {missingGradeStudents.length} {missingGradeStudents.length===1?"elev":"elever"}.</strong><br />De kan stadig få opgaven, men klassetrinstilpasningen bliver først aktiv, når klassetrinnet er sat. <a href="/admin/student-grade-levels" style={{color:"#526b60",fontWeight:900}}>Sæt klassetrin →</a></div>}<div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",marginTop:18}}><button disabled={saving||(recipientMode==="students"&&!selectedStudents.length)} onClick={createAssignment} style={{padding:"12px 18px",border:0,borderRadius:9,background:"#365044",color:"white",fontWeight:900,cursor:saving?"wait":"pointer",opacity:saving||(recipientMode==="students"&&!selectedStudents.length)?0.55:1}}>{saving?"Tildeler…":"Tildel nu →"}</button></div></div></div>}
   ...
   {assignments.length===0?<p style={{color:"#777"}}>Ingen grammatikopgaver tildelt endnu.</p>:<div style={{display:"grid",gap:9,marginTop:15}}>{assignments.slice(0,10).map(a=>{
    ...
    return <article key={a.id} style={{border:"1px solid #e4e0d8",borderRadius:10,background:openAssignment===a.id?"#edf1ec":"#faf9f6",padding:13}}>
     {editing&&!locked?<div style={{display:"grid",gap:9}}><strong>Redigér grammatiktildeling</strong><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:8}}><label style={{fontWeight:800,fontSize:12}}>Område<select value={editArea} onChange={e=>{setEditArea(e.target.value);setEditTopic("")}} style={field}>{areas.map(x=><option key={x.title} value={x.title}>{x.title}</option>)}</select></label><label style={{fontWeight:800,fontSize:12}}>Emne<select value={editTopic} onChange={e=>setEditTopic(e.target.value)} style={field}><option value="">Vælg emne</option>{(areas.find(x=>x.title===editArea)?.topics||[]).map(x=><option key={x}>{x}</option>)}</select></label><label style={{fontWeight:800,fontSize:12}}>Niveau<select value={editLevel} onChange={e=>setEditLevel(e.target.value)} style={field}>{levels.map(x=><option key={x.id} value={x.id}>{x.title}</option>)}</select></label><label style={{fontWeight:800,fontSize:12}}>Klasse<select value={editClassId} onChange={e=>{setEditClassId(Number(e.target.value));setEditStudents([]);setEditRecipientMode("class")}} style={field}>{classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label></div><div><strong style={{fontSize:12}}>Modtagere</strong><div style={{display:"flex",gap:7,flexWrap:"wrap",marginTop:7}}><button onClick={()=>{setEditRecipientMode("class");setEditStudents([])}} style={button(editRecipientMode==="class")}>Hele klassen</button><button onClick={()=>setEditRecipientMode("students")} style={button(editRecipientMode==="students")}>Udvalgte elever</button></div>{editRecipientMode==="students"&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}>{editClassStudents.map(s=><button key={s.id} onClick={()=>toggleEditStudent(s.id)} style={{...smallButton,borderRadius:999,background:editStudents.includes(s.id)?"#edf1ec":"white",border:editStudents.includes(s.id)?"2px solid #526b60":"1px solid #d8d5cd"}}>{editStudents.includes(s.id)?"✓ ":""}{s.name}{s.grade_level!==null&&s.grade_level!==undefined?` · ${s.grade_level}.`:" · ?"}</button>)}</div>}</div><div style={{display:"flex",gap:7,flexWrap:"wrap"}}><button onClick={()=>saveEdit(a)} disabled={saving||!editTopic||(editRecipientMode==="students"&&!editStudents.length)} style={{...smallButton,background:"#365044",color:"white",borderColor:"#365044"}}>{saving?"Gemmer…":"Gem"}</button><button onClick={()=>setEditId(null)} style={smallButton}>Annullér</button><button onClick={()=>removeAssignment(a)} style={{...smallButton,color:"#8a3c34"}}>Slet</button></div></div>:<>
      ...
      {openAssignment===a.id&&<div style={{padding:"14px 2px 2px",borderTop:"1px solid #dedfd9",marginTop:12}}>{resultsLoading?<p>Henter status…</p>:results.length===0?<p style={{color:"#777"}}>Ingen elevstatus endnu.</p>:<><div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}><span style={statusChip}>{results.filter(r=>r.completed).length} har forsøgt</span><span style={statusChip}>{results.filter(r=>r.mastered).length} mestret</span><span style={statusChip}>{results.filter(r=>!r.completed).length} mangler</span><span style={statusChip}>{results.length} tildelt</span></div><div style={{display:"grid",gap:7}}>{results.map(r=>{
       const history=Array.isArray(r.attempt_history)?r.attempt_history:[];
       return <div key={r.student_id} style={{background:"white",border:"1px solid #e3dfd7",borderRadius:9,padding:"10px 12px"}}><button onClick={()=>setOpenStudent(openStudent===r.student_id?null:r.student_id)} style={{width:"100%",display:"flex",justifyContent:"space-between",gap:10,border:0,background:"transparent",padding:0,textAlign:"left",cursor:"pointer",color:"inherit"}}><strong>{r.student_name}</strong><span style={{fontWeight:800,color:r.mastered?"#42614f":r.completed?"#8a6e42":"#8a6e42"}}>{r.completed?`${r.score??0}/${r.max_score??0} senest · ${r.attempts??1} forsøg`:"Mangler"}</span></button>{openStudent===r.student_id&&r.completed&&<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid #eee"}}><div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:10}}><span style={statusChip}>Bedste: {r.best_score??r.score??0}/{r.best_max_score??r.max_score??0}</span><span style={statusChip}>Seneste: {r.score??0}/{r.max_score??0}</span><span style={statusChip}>Forsøg: {r.attempts??1}</span>{r.mastered&&<span style={{...statusChip,background:"#e7eee9",color:"#42614f"}}>Mestret ✓</span>}</div>{history.length>0?<div style={{display:"grid",gap:9}}><div><strong style={{fontSize:13}}>Forsøgsudvikling</strong><div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:7}}>{history.map((h,index)=><span key={h.attempt_number} style={{...statusChip,background:h.score===h.max_score?"#e7eee9":"#f2eee5"}}>#{h.attempt_number}: {h.score}/{h.max_score}{index<history.length-1?" →":""}</span>)}</div></div>{history.map(h=><details key={h.attempt_number} style={{border:"1px solid #e5e1d8",borderRadius:8,background:"#faf9f6",padding:"9px 11px"}}><summary style={{cursor:"pointer",fontWeight:850}}>Forsøg {h.attempt_number} · {h.score}/{h.max_score}</summary>{h.answers&&<div style={{display:"grid",gap:7,marginTop:10}}>{Object.entries(h.answers).map(([i,v])=>renderAnswer(`${h.attempt_number}-${i}`,v))}</div>}</details>)}</div>:<>{(r.attempts??0)>0&&<div style={{padding:"9px 11px",borderRadius:8,background:"#f7f3e9",fontSize:12,color:"#6d6657",marginBottom:r.answers?10:0}}>Detaljeret rundehistorik registreres fra nu af. Tidligere forsøg kan ikke genskabes sikkert.</div>}{r.answers&&<div style={{display:"grid",gap:7}}>{Object.entries(r.answers).map(([i,v])=>renderAnswer(i,v))}</div>}</>}</div>}</div>;
      })}</div></>}</div>}
     </>}
    </article>;
   })}</div>}
