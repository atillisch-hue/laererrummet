...
import { classReadingFocus, classReadingStrategyAnalysis, type ClassStrategyRow } from "./class-reading-analysis";
...
type StrategyAssignment = { id: number; title: string; strategy: string; target_grade: number; created_at: string; recipient_count: number; completed_count: number; locked: boolean };
...
function heatStatus(status: "focus" | "watch" | "secure") { if (status === "focus") return { label: "Fælles fokus", symbol: "●", background: "#fff0ed", border: "#e2b4ad", color: "#8c443b" }; if (status === "watch") return { label: "Hold øje", symbol: "●", background: "#fff7e8", border: "#ead8ad", color: "#7a6031" }; return { label: "Sikker", symbol: "●", background: "#edf5ef", border: "#cadfce", color: "#42614f" }; }
...
export default function ReadingExamTeacher() {
  ...
  const [assignments, setAssignments] = useState<ExamAssignment[]>([]), [strategyAssignments, setStrategyAssignments] = useState<StrategyAssignment[]>([]), [message, setMessage] = useState(""), [saving, setSaving] = useState(false), [assigningStrategy, setAssigningStrategy] = useState(""), [open, setOpen] = useState<number | null>(null), [results, setResults] = useState<ExamResult[]>([]), [loadingResults, setLoadingResults] = useState(false);
  ...
  async function loadAssignments(id: number) {
    const [examResponse, strategyResponse] = await Promise.all([
      supabase.rpc("teacher_reading_exam_assignments", { p_class_id: id }),
      supabase.rpc("teacher_reading_strategy_assignments", { p_class_id: id }),
    ]);
    if (examResponse.error || !examResponse.data?.ok) { setAssignments([]); if (examResponse.error) setMessage(examResponse.error.message); }
    else setAssignments(examResponse.data.assignments || []);
    if (strategyResponse.error || !strategyResponse.data?.ok) setStrategyAssignments([]);
    else setStrategyAssignments(strategyResponse.data.assignments || []);
  }

  useEffect(() => { (async () => {
    const { data: s } = await supabase.auth.getSession(); if (!s.session) { window.location.href = "/?teacher=1"; return; }
    const [c, st] = await Promise.all([supabase.from("classes").select("id,name").order("id"), supabase.from("students").select("id,name,class_id,grade_level").order("name")]);
  ...
  async function create() {
    ...
    const { data, error } = await supabase.rpc("create_reading_exam_assignment", { p_class_id: Number(classId), p_title: title.trim() || "Træn læseprøven", p_time_limit_minutes: timed ? minutes : null, p_student_ids: mode === "students" ? selected : null, p_target_grade: targetGrade });
    if (error || !data?.ok) setMessage(`Kunne ikke tildele: ${error?.message || data?.error || "ukendt fejl"}`);
    else { setMessage(`✓ Læseprøven på ${targetGrade}.-klasseniveau er tildelt ${mode === "class" ? "hele klassen" : `${selected.length} elev${selected.length === 1 ? "" : "er"}`}.`); await loadAssignments(Number(classId)); }
    setSaving(false);
  }

  async function assignStrategy(row: ClassStrategyRow, grade: number) {
    if (!classId || !row.supportStudents.length || assigningStrategy) return;
    setAssigningStrategy(row.strategy); setMessage("");
    const recipients = row.supportStudents.map((student) => student.id);
    const { data, error } = await supabase.rpc("create_reading_strategy_assignment", { p_class_id: Number(classId), p_strategy: row.strategy, p_target_grade: grade, p_student_ids: recipients, p_title: `Træn ${row.strategy}` });
    if (error || !data?.ok) setMessage(`Kunne ikke tildele strategitræningen: ${error?.message || data?.error || "ukendt fejl"}`);
    else { setMessage(`✓ ${row.strategy} er tildelt ${recipients.length} elev${recipients.length === 1 ? "" : "er"} på ${grade}.-klasseniveau.`); await loadAssignments(Number(classId)); }
    setAssigningStrategy("");
  }

  async function remove(a: ExamAssignment) {
    if (a.locked) { setMessage("Prøven er startet af mindst én elev og er derfor låst som historik."); return; }
    ...
    if (error || !data?.ok) setMessage(error?.message || data?.error || "Kunne ikke slette"); else { if (open === a.id) { setOpen(null); setResults([]); } await loadAssignments(Number(classId)); }
  }

  async function removeStrategyAssignment(a: StrategyAssignment) {
    if (a.locked) { setMessage("Mindst én elev har allerede arbejdet med denne strategitildeling, så den bevares som historik."); return; }
    if (!confirm(`Vil du slette strategitræningen “${a.strategy}”?`)) return;
    const { data, error } = await supabase.rpc("delete_reading_strategy_assignment", { p_assignment_id: a.id });
    if (error || !data?.ok) setMessage(data?.error === "assignment_started" ? "Strategitræningen er allerede startet og kan ikke slettes." : error?.message || data?.error || "Kunne ikke slette"); else await loadAssignments(Number(classId));
  }
  ...
  return <main style={{ minHeight: "100vh", background: "#f5f3ee", padding: "30px 24px 80px", color: "#26342e" }}><section style={{ maxWidth: 1080, margin: "auto" }}>
    ...
    <div style={{ ...panel, marginTop: 22 }}><p style={eyebrow}>OPFØLGNING</p><h2 style={h2}>Tildelte læseprøver</h2>{assignments.length === 0 ? <p style={{ color: "#777" }}>Ingen læseprøver tildelt endnu.</p> : <div style={{ display: "grid", gap: 9 }}>{assignments.map((a) => <article key={a.id} style={{ border: "1px solid #e3dfd7", borderRadius: 10, padding: 13, background: open === a.id ? "#edf1ec" : "#faf9f6" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}><button onClick={() => showResults(a.id)} style={{ border: 0, background: "transparent", padding: 0, textAlign: "left", cursor: "pointer", color: "inherit", flex: "1 1 360px" }}><strong style={{ display: "block" }}>{a.title}</strong><span style={{ fontSize: 12, color: "#737873" }}>{a.target_grade}. kl. · {a.question_count} delspørgsmål · {a.time_limit_minutes ? `${a.time_limit_minutes} min` : "uden tid"} · {a.submitted_count}/{a.recipient_count} afleveret</span></button><div style={{ display: "flex", gap: 7 }}>{!a.locked && <button onClick={() => remove(a)} style={{ ...smallButton, color: "#8a3c34" }}>Slet</button>}<button onClick={() => showResults(a.id)} style={smallButton}>{open === a.id ? "Luk ↑" : "Se status →"}</button></div></div>
      {open === a.id && <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #d9ddd8" }}>{loadingResults ? <p>Henter resultater…</p> : <div style={{ display: "grid", gap: 12 }}>
        {submittedResults.length > 0 && <section style={{ background: "white", border: "1px solid #d8d5cd", borderRadius: 12, padding: 15 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "start" }}><div><p style={eyebrow}>KLASSENS LÆSEPROFIL</p><h3 style={{ fontFamily: "Georgia,serif", fontSize: 24, margin: "5px 0" }}>Hvad skal vi arbejde med sammen?</h3></div><span style={chip}>{a.submitted_count}/{a.recipient_count} afleveret</span></div>
          ...
          {sharedFocus ? <div style={{ marginTop: 12, padding: 14, borderRadius: 10, background: "#fff7e8", border: "1px solid #ead8ad" }}><p style={{ ...eyebrow, color: "#7a6031" }}>ANBEFALET FÆLLES UNDERVISNINGSFOKUS</p><strong style={{ display: "block", fontFamily: "Georgia,serif", fontSize: 22, marginTop: 5 }}>{sharedFocus.strategy}</strong><p style={{ margin: "6px 0", lineHeight: 1.5 }}>{READING_STRATEGY_COACHING[sharedFocus.strategy].title}. {READING_STRATEGY_COACHING[sharedFocus.strategy].explanation}</p><p style={{ margin: "6px 0 0", fontWeight: 850 }}>Forslag til næste undervisningsgreb: {READING_STRATEGY_COACHING[sharedFocus.strategy].move}</p></div> : <div style={{ marginTop: 12, padding: 13, borderRadius: 10, background: "#edf5ef", border: "1px solid #cadfce", color: "#42614f", fontWeight: 850 }}>Klassen ligger på mindst 80 % i alle målte strategier. Der er ikke ét tydeligt fælles problemområde ✓</div>}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 9, marginTop: 13 }}>{classStrategyRows.filter((row) => row.total > 0).map((row) => { const state = heatStatus(row.status), percent = Math.round(row.accuracy * 100); return <article key={row.strategy} style={{ border: `1px solid ${state.border}`, background: state.background, borderRadius: 10, padding: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "start" }}><strong>{row.strategy}</strong><span style={{ fontSize: 11, fontWeight: 900, color: state.color, whiteSpace: "nowrap" }}>{state.symbol} {state.label}</span></div><div style={{ fontFamily: "Georgia,serif", fontSize: 26, fontWeight: 900, marginTop: 8, color: state.color }}>{percent}%</div><div style={{ fontSize: 12, color: "#66706a" }}>{row.correct}/{row.total} svar · {row.measuredStudents} elev{row.measuredStudents === 1 ? "" : "er"} målt</div>{row.supportStudents.length > 0 && <><details style={{ marginTop: 9 }}><summary style={{ cursor: "pointer", fontSize: 12, fontWeight: 850, color: state.color }}>{row.supportStudents.length} elev{row.supportStudents.length === 1 ? "" : "er"} under 70 %</summary><div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginTop: 7 }}>{row.supportStudents.map((student) => <span key={student.id} style={{ ...chip, background: "white" }}>{student.name}: {student.correct}/{student.total}</span>)}</div></details><button onClick={() => assignStrategy(row, a.target_grade)} disabled={Boolean(assigningStrategy)} style={{ ...smallButton, marginTop: 9, width: "100%", color: state.color, opacity: assigningStrategy ? .55 : 1 }}>{assigningStrategy === row.strategy ? "Tildeler…" : `Tildel ${row.strategy} til de ${row.supportStudents.length} elever →`}</button></>}</article>; })}</div>
        </section>}
        <div style={{ display: "grid", gap: 8 }}>{results.map((r) => { const percent = r.submitted && r.max_score ? Math.round((Number(r.score || 0) / r.max_score) * 100) : null; const focus = r.submitted && r.answers ? weakestReadingStrategy(readingStrategyScoresFromSnapshot(r.answers)) : null; return <div key={r.student_id} style={{ background: "white", border: "1px solid #e2ded6", borderRadius: 9, padding: 11 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}><strong>{r.student_name}</strong><span style={{ fontWeight: 850, color: r.submitted ? "#42614f" : r.started ? "#8a6e42" : "#777" }}>{r.submitted ? `${r.score}/${r.max_score} · ${percent}% · ${formatTime(r.elapsed_seconds)}` : r.started ? `I gang · ${formatTime(r.elapsed_seconds)}` : "Ikke startet"}{r.timed_out ? " · tiden udløb" : ""}</span></div>{r.submitted && r.answers && <><div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}>{strategySummary(r.answers).map(([strategy, row]) => <span key={strategy} style={chip}>{strategy}: {row.correct}/{row.total}</span>)}</div><div style={{ marginTop: 9, padding: "8px 10px", borderRadius: 8, background: focus ? "#fff7e8" : "#edf5ef", fontSize: 13, fontWeight: 800, color: focus ? "#6b572f" : "#42614f" }}>{focus ? <>Næste fokus: <strong>{focus.strategy}</strong> · {focus.correct}/{focus.total}</> : <>Alle målte strategier sikre ✓</>}</div></>}</div>; })}</div>
      </div>}</div>}
    </article>)}</div>}</div>

    <div style={{ ...panel, marginTop: 22 }}><p style={eyebrow}>MÅLRETTET OPFØLGNING</p><h2 style={h2}>Tildelt strategitræning</h2><p style={{ color: "#6d746f", marginTop: -7 }}>Disse korte træninger er sendt direkte fra læseprofilen. En tildeling tæller som gennemført, når eleven har lavet en ny 3-opgavers runde efter tildelingstidspunktet.</p>{strategyAssignments.length === 0 ? <p style={{ color: "#777" }}>Ingen målrettede strategitildelinger endnu.</p> : <div style={{ display: "grid", gap: 8 }}>{strategyAssignments.map((a) => <div key={a.id} style={{ border: "1px solid #e2ded6", borderRadius: 10, padding: 12, background: "#faf9f6", display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}><div><strong>{a.strategy}</strong><div style={{ fontSize: 12, color: "#737873", marginTop: 3 }}>{a.target_grade}. kl. niveau · {a.completed_count}/{a.recipient_count} gennemført</div></div>{!a.locked && <button onClick={() => removeStrategyAssignment(a)} style={{ ...smallButton, color: "#8a3c34" }}>Slet</button>}</div>)}</div>}</div>
  </section></main>;
}
