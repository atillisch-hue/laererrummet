...
import {danishCommunicationTemplates,danishCommunicationByName,type DanishCommunicationTemplate} from "../../lib/danishCommunicationCatalog";
...
type DanishMode="writing"|"analysis"|"communication";
...
function requestedDanishMode(value:string|null):DanishMode{return value==="analysis"?"analysis":value==="communication"?"communication":"writing"}
function assignmentKindForSubject(slug:string|undefined,danishMode:DanishMode):AssignmentKind{
 if(slug==="dansk")return danishMode==="analysis"?"danish_analysis":danishMode==="communication"?"danish_communication":"danish_writing";
...
export default function AssignmentEditor(){
 ...
 const[classId,setClassId]=useState<number|"">(""),[roomId,setRoomId]=useState<number|"">(""),[title,setTitle]=useState(""),[instructions,setInstructions]=useState(""),[genreId,setGenreId]=useState("debatindlaeg"),[category,setCategory]=useState<DanishGenreCategory>("Opinion"),[analysisId,setAnalysisId]=useState("tekstspor"),[communicationId,setCommunicationId]=useState("mundtlig-fortaelling"),[danishMode,setDanishMode]=useState<DanishMode>(requestedDanishMode(search.get("kind"))),[templateId,setTemplateId]=useState(""),[saving,setSaving]=useState(false),[recipientMode,setRecipientMode]=useState<"class"|"students">("class"),[selected,setSelected]=useState<number[]>([]);
 ...
 useEffect(()=>{(async()=>{
  ...
  if(editing){
   ...
   }else if(row.assignment_kind==="danish_communication"){
    setDanishMode("communication");const found=danishCommunicationByName(row.type);if(found)setCommunicationId(found.id);
 ...
 const communicationTemplate:DanishCommunicationTemplate=useMemo(()=>danishCommunicationTemplates.find(x=>x.id===communicationId)||danishCommunicationTemplates[0],[communicationId]);
 ...
 const taskType=assignmentKind==="danish_analysis"?analysisTemplate.name:assignmentKind==="danish_communication"?communicationTemplate.name:assignmentKind==="danish_writing"?genre.name:template?.name||"Opgave";
 ...
 const belowCommunicationStart=assignmentKind==="danish_communication"?recipientStudents.filter(s=>s.grade_level!=null&&s.grade_level<communicationTemplate.minGrade).length:0;
 ...
 const heading=subjectSlug==="dansk"?(danishMode==="analysis"?"Dansk tekstarbejde":danishMode==="communication"?"Dansk kommunikationsopgave":"Danskopgave"):subjectSlug==="matematik"?"Matematikopgave":currentSubject?`${currentSubject.name}-opgave`:"Ny opgave";
 const helper=subjectSlug==="dansk"?(danishMode==="analysis"?"Vælg et analytisk fokus. Eleven får tekstnære arbejdsspørgsmål, som tilpasses elevens klassetrin.":danishMode==="communication"?"Vælg en mundtlig eller multimodal arbejdsform. Eleven får en plan med stikord, formål, modtager og øvepunkter tilpasset klassetrin.":"Vælg en dansk teksttype. Eleven får genretilpasset skrivehjælp."):subjectSlug==="matematik"?"Vælg en matematisk arbejdsform. Eleven får felter, der gør metode, beregning og ræsonnement synligt.":"Vælg en faglig arbejdsform, der passer til opgaven.";
 const titlePlaceholder=assignmentKind==="danish_analysis"?"Fx Undersøg fortæller og synsvinkel i teksten":assignmentKind==="danish_communication"?"Fx Hold en kort faglig præsentation om kunstig intelligens":subjectSlug==="matematik"?"Fx Design den billigste skoleudflugt":`Fx ${taskType} om kunstig intelligens`;
 const instructionsPlaceholder=assignmentKind==="danish_analysis"?"Skriv hvilken tekst eleven skal arbejde med, og hvad der særligt skal undersøges.":assignmentKind==="danish_communication"?"Skriv hvad eleven skal formidle, til hvem, i hvilket format og eventuelle rammer for tid eller produkt.":"Skriv hvad eleven skal undersøge, løse, skrive eller producere.";
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",padding:"28px 24px 80px",color:"#26342e"}}><section style={{maxWidth:1040,margin:"0 auto"}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"minmax(0,1.25fr) minmax(280px,.75fr)",gap:14}}>
   <div style={{background:"white",border:"1px solid #dfdcd4",borderRadius:13,padding:20}}>
    ...
    <label style={{display:"block",fontWeight:800,fontSize:13,marginTop:16}}>Opgaveformulering<textarea value={instructions} onChange={e=>setInstructions(e.target.value)} placeholder={instructionsPlaceholder} rows={6} style={{...input,lineHeight:1.5,resize:"vertical"}}/><span style={{display:"block",fontWeight:400,...small,marginTop:5}}>Det er denne tekst, eleven ser, når opgaven åbnes.</span></label>
   ...
   <aside style={{background:"#e8eee9",border:"1px solid #d4ddd6",borderRadius:13,padding:18,alignSelf:"start"}}><p style={{fontSize:11,fontWeight:900,letterSpacing:1.2,color:"#65766d",margin:"0 0 4px"}}>ELEVEN FÅR</p><h2 style={{fontFamily:"Georgia,serif",fontSize:23,margin:"5px 0"}}>{taskType}</h2>{assignmentKind==="danish_analysis"?<><p style={{...small,lineHeight:1.5}}>{analysisTemplate.description}</p><div style={previewTip}><strong>Arbejdsprincip:</strong> tekstspor → iagttagelse → forklaring → fortolkning med belæg.</div><strong style={{display:"block",marginTop:14}}>Arbejdsspørgsmål</strong><ol style={previewList}>{analysisTemplate.prompts.map(x=><li key={x}>{x}</li>)}</ol><small style={previewNote}>Spørgsmålene bliver automatisk gjort enklere eller mere krævende efter elevens klassetrin.</small></>:assignmentKind==="danish_communication"?<><p style={{...small,lineHeight:1.5}}>{communicationTemplate.description}</p><div style={previewTip}><strong>Arbejdsprincip:</strong> formål → modtager → stikord → øv → lyt og tilpas.</div><strong style={{display:"block",marginTop:14}}>Planlægningsfelter</strong><ol style={previewList}>{communicationTemplate.planning.map(x=><li key={x}>{x}</li>)}</ol><small style={previewNote}>Eleven får støtte til planlægning og øvning — ikke et manuskript, der skal læses op.</small></>:assignmentKind==="danish_writing"?<><p style={{...small,lineHeight:1.5}}><strong>Formål:</strong> {genre.purpose}</p><p style={{...small,lineHeight:1.5}}><strong>Modtager:</strong> {genre.audience}</p><strong style={{display:"block",marginTop:14}}>Skrivefelter</strong><ol style={previewList}>{genre.structure.map(x=><li key={x}>{x}</li>)}</ol></>:template?<><p style={{...small,lineHeight:1.5}}>{template.description}</p><div style={previewTip}><strong>Arbejdsråd:</strong> {template.coach}</div><strong style={{display:"block",marginTop:14}}>Arbejdsfelter</strong><ol style={previewList}>{template.structure.map(x=><li key={x}>{x}</li>)}</ol></>:<p style={small}>Vælg først et faglokale.</p>}</aside>
  </div>
 </section></main>;
}

function ProgressionNotice({count,label}:{count:number;label:string}){return <div style={{marginTop:12,padding:"10px 12px",borderRadius:9,background:"#fff3cd",fontSize:12,lineHeight:1.45}}><strong>{count} elev{count===1?"":"er"}</strong> ligger under {label}s normale starttrin. Du kan stadig tildele det; eleven får det enklere klassetrinsstillads.</div>}
...
const gradeHint:React.CSSProperties={display:"inline-block",fontSize:10,fontWeight:900,marginTop:7,padding:"3px 6px",borderRadius:999,background:"#eee9dd",color:"#756548"};
const previewTip:React.CSSProperties={padding:10,borderRadius:9,background:"white",fontSize:12,lineHeight:1.45};
const previewList:React.CSSProperties={paddingLeft:20,...small,lineHeight:1.7};
const previewNote:React.CSSProperties={display:"block",marginTop:8,color:"#647169"};
