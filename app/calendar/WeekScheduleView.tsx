...
export default function WeekScheduleView({selectedDate,onSelectDate,viewedUserId,currentUserId,viewedName,meetings}:Props){
 ...
 useEffect(()=>{
  ...
  (async()=>{
   ...
   const[eRes,tRes,cRes,bRes]=await Promise.all([
   ...
   const problem=eRes.error||tRes.error||cRes.error||bRes.error;
   if(problem)setError(problem.message||"Ugen kunne ikke hentes helt.");
   setLoading(false);
  })();
  return()=>{active=false};
 },[viewedUserId,weekStart,weekEnd]);
 ...
 function meetingsForDate(date:string){
  return meetings.filter(m=>m.starts_at.slice(0,10)===date).sort((a,b)=>a.starts_at.localeCompare(b.starts_at));
 }
 function busyForDate(date:string){
  const start=new Date(`${date}T00:00:00`).getTime(),end=new Date(`${date}T23:59:59`).getTime();
  return busy.filter(x=>new Date(x.starts_at).getTime()<=end&&new Date(x.ends_at).getTime()>=start);
 }
 function entriesForDate(date:string){
  const weekday=new Date(`${date}T12:00:00`).getDay();
  return entries.filter(e=>assignedIds.has(e.id)&&e.weekday===weekday&&scheduleOccursOn(e.recurrence_pattern,date)).sort((a,b)=>a.start_time.localeCompare(b.start_time));
 }
 ...
 return <section style={{background:"white",border:"1px solid #ddd9d0",borderRadius:16,padding:18}}>
  ...
  {loading?<div style={{padding:"28px 4px",color:"#727772"}}>Henter ugeplan…</div>:<div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(155px,1fr))",gap:8,marginTop:16,overflowX:"auto",paddingBottom:4}}>
   {days.map(day=>{const date=iso(day),selected=date===selectedDate,today=date===iso(new Date()),dayEntries=entriesForDate(date),dayBusy=busyForDate(date),absence=dayBusy.some(x=>x.busy_type==="absence"),ownMeetings=isSelf?meetingsForDate(date):[];return <article key={date} onClick={()=>onSelectDate(date)} style={{border:selected?"2px solid #486b59":"1px solid #e1ddd4",borderRadius:12,padding:10,background:selected?"#f3f7f3":today?"#faf7ed":"#faf9f6",minHeight:270,cursor:"pointer"}}>
    ...
    <div style={{display:"grid",gap:6,marginTop:9}}>
     ...
     {!absence&&dayEntries.length===0&&(!isSelf?dayBusy.filter(x=>x.busy_type==="meeting").length===0:ownMeetings.length===0)&&<small style={{color:"#929790",padding:"8px 2px"}}>Ingen skemaposter</small>}
    </div>
   </article>})}
  </div>}
 </section>;
