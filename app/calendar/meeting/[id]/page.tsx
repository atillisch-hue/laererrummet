...
export default function MeetingPage({params}:{params:Promise<{id:string}>}){
 ...
 async function toggleAction(a:Action){if(!canManage)return;const{error:e}=await supabase.from("meeting_actions").update({completed:!a.completed,completed_at:!a.completed?new Date().toISOString():null}).eq("id",a.id);if(e)setError(e.message);else await load()}
 ...
 async function saveActionEdit(id:number){if(!editActionTitle.trim())return;const{error:e}=await supabase.from("meeting_actions").update({title:editActionTitle.trim(),responsible_user_id:editActionResponsible||null,due_date:editActionDue||null}).eq("id",id);if(e){setError(e.message);return}setEditingActionId(null);await load()}
 async function removeAction(id:number){if(!window.confirm("Vil du slette denne handling?"))return;const{error:e}=await supabase.from("meeting_actions").delete().eq("id",id);if(e)setError(e.message);else await load()}
 async function addDecision(){if(!decision.trim())return;const{data:{user}}=await supabase.auth.getUser();if(!user)return;const{error:e}=await supabase.from("meeting_decisions").insert({meeting_id:meetingId,text:decision.trim(),created_by:user.id});if(e){setError(e.message);return}setDecision("");await load()}
 function startEditDecision(d:Decision){setEditingDecisionId(d.id);setEditDecisionText(d.text)}
 async function saveDecisionEdit(id:number){if(!editDecisionText.trim())return;const{error:e}=await supabase.from("meeting_decisions").update({text:editDecisionText.trim()}).eq("id",id);if(e){setError(e.message);return}setEditingDecisionId(null);await load()}
 async function removeDecision(id:number){if(!window.confirm("Vil du slette denne beslutning?"))return;const{error:e}=await supabase.from("meeting_decisions").delete().eq("id",id);if(e)setError(e.message);else await load()}
