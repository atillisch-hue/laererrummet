...
export default function MeetingPage({params}:{params:Promise<{id:string}>}){
 ...
 const[meeting,setMeeting]=useState<Meeting|null>(null),[agenda,setAgenda]=useState(""),[minutes,setMinutes]=useState(""),[internalNotes,setInternalNotes]=useState(""),[actions,setActions]=useState<Action[]>([]),[decisions,setDecisions]=useState<Decision[]>([]),[participants,setParticipants]=useState<Participant[]>([]),[directory,setDirectory]=useState<User[]>([]),[newAction,setNewAction]=useState(""),[responsible,setResponsible]=useState(""),[due,setDue]=useState(""),[decision,setDecision]=useState(""),[saving,setSaving]=useState(false),[error,setError]=useState(""),[canManage,setCanManage]=useState(false),[editing,setEditing]=useState(false),[title,setTitle]=useState(""),[meetingType,setMeetingType]=useState(""),[startsAt,setStartsAt]=useState(""),[endsAt,setEndsAt]=useState(""),[location,setLocation]=useState(""),[status,setStatus]=useState("planned");
 ...
 async function saveNotes(){setSaving(true);setError("");const{error:e}=await supabase.from("calendar_meetings").update({agenda,minutes,internal_notes:internalNotes}).eq("id",meetingId);if(e)setError("Du har ikke ret til at gemme ændringer på dette møde.");else await load();setSaving(false)}
 async function removeMeeting(){
  if(!canManage)return;
  const ok=window.confirm("Slet mødet permanent? Deltagere, bookinger, beslutninger og handlinger knyttet til mødet bliver også slettet.");if(!ok)return;
  setSaving(true);setError("");
  const{error:e}=await supabase.from("calendar_meetings").delete().eq("id",meetingId);
  if(e){setError(e.message);setSaving(false);return}
  window.location.replace("/calendar");
 }
 async function addAction(){if(!newAction.trim())return;const{data:{user}}=await supabase.auth.getUser();const{error:e}=await supabase.from("meeting_actions").insert({meeting_id:meetingId,title:newAction.trim(),due_date:due||null,responsible_user_id:responsible||null,created_by:user?.id});if(!e){setNewAction("");setDue("");setResponsible("");await load()}}
 ...
 async function addDecision(){if(!decision.trim())return;const{data:{user}}=await supabase.auth.getUser();if(!user)return;const{error:e}=await supabase.from("meeting_decisions").insert({meeting_id:meetingId,text:decision.trim(),created_by:user.id});if(!e){setDecision("");await load()}}
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",color:"#26342e"}}>
  ...
  <section style={shell}>
   {editing&&canManage&&<section style={{...card,marginBottom:16,border:"1px solid #b9cabb",background:"#f7faf7"}}><p style={eyebrow}>REDIGÉR MØDE</p><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:10,marginTop:12}}><label style={label}>Titel<input value={title} onChange={e=>setTitle(e.target.value)} style={input}/></label><label style={label}>Type<input value={meetingType} onChange={e=>setMeetingType(e.target.value)} style={input}/></label><label style={label}>Start<input type="datetime-local" value={startsAt} onChange={e=>setStartsAt(e.target.value)} style={input}/></label><label style={label}>Slut<input type="datetime-local" value={endsAt} onChange={e=>setEndsAt(e.target.value)} style={input}/></label><label style={label}>Lokale / sted<input value={location} onChange={e=>setLocation(e.target.value)} style={input}/></label><label style={label}>Status<select value={status} onChange={e=>setStatus(e.target.value)} style={input}><option value="planned">Planlagt</option><option value="completed">Afsluttet</option><option value="cancelled">Aflyst</option></select></label></div><div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:14}}><button onClick={saveDetails} disabled={saving||!title.trim()||!startsAt} style={secondary}>{saving?"Gemmer…":"Gem ændringer"}</button><button onClick={()=>{setEditing(false);setTitle(meeting.title);setMeetingType(meeting.meeting_type);setStartsAt(toLocalInput(meeting.starts_at));setEndsAt(toLocalInput(meeting.ends_at));setLocation(meeting.location||"");setStatus(meeting.status)}} style={cancelButton}>Annuller</button></div></section>}
   ...
   <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:16,marginTop:16}}><section style={card}><h2 style={h2}>Beslutninger</h2>{decisions.map(d=><div key={d.id} style={row}>✓ {d.text}</div>)}{!decisions.length&&<p style={hint}>Ingen beslutninger endnu.</p>}{canManage&&<><textarea value={decision} onChange={e=>setDecision(e.target.value)} placeholder="Hvad besluttede I?" style={{...textarea,minHeight:80,marginTop:12}}/><button onClick={addDecision} style={secondary}>+ Tilføj beslutning</button></>}</section><section style={card}><h2 style={h2}>Handlinger</h2>{actions.map(a=><button key={a.id} onClick={()=>canManage&&toggleAction(a)} style={{...row,width:"100%",textAlign:"left",border:0,cursor:canManage?"pointer":"default",opacity:a.completed?.65:1,textDecoration:a.completed?"line-through":"none"}}><b>{a.completed?"☑ ":"☐ "}</b>{a.title}{a.responsible_user_id&&<small style={{display:"block",marginLeft:24}}>Ansvarlig: {nameFor(a.responsible_user_id)}</small>}{a.due_date&&<small style={{display:"block",marginLeft:24}}>Deadline {new Date(a.due_date+"T12:00").toLocaleDateString("da-DK")}</small>}</button>)}{!actions.length&&<p style={hint}>Ingen handlinger endnu.</p>}{canManage&&<><input value={newAction} onChange={e=>setNewAction(e.target.value)} placeholder="Hvad skal gøres?" style={input}/><select value={responsible} onChange={e=>setResponsible(e.target.value)} style={input}><option value="">Ingen ansvarlig valgt</option>{staff.map(p=><option key={p.id} value={p.user_id||""}>{nameFor(p.user_id)}</option>)}</select><input type="date" value={due} onChange={e=>setDue(e.target.value)} style={input}/><button onClick={addAction} style={secondary}>+ Tilføj handling</button></>}</section></div>
  </section>
 </main>
}
...
const label:React.CSSProperties={fontSize:13,fontWeight:900,color:"#4d5c53"};
