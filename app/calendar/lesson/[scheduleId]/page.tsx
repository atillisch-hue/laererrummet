...
export default function LessonWorkRoom(){
 ...
 const[ready,setReady]=useState(false),[entry,setEntry]=useState<Entry|null>(null),[klass,setKlass]=useState<Klass|null>(null),[lessonId,setLessonId]=useState<number|null>(null),[goals,setGoals]=useState(""),[plan,setPlan]=useState(""),[materialsText,setMaterialsText]=useState(""),[status,setStatus]=useState<Lesson["status"]>("planned"),[startedAt,setStartedAt]=useState<string|null>(null),[endedAt,setEndedAt]=useState<string|null>(null),[carryNote,setCarryNote]=useState(""),[carriedTo,setCarriedTo]=useState<string|null>(null),[carryFrom,setCarryFrom]=useState<CarryFrom|null>(null),[canEdit,setCanEdit]=useState(false),[message,setMessage]=useState(""),[saving,setSaving]=useState(false);
 ...
 const nextLessonDate=useMemo(()=>{if(!validDate)return "";const d=new Date(lessonDate+"T12:00:00");d.setDate(d.getDate()+7);return dateOnly(d)},[lessonDate,validDate]);
 ...
 useEffect(()=>{
  ...
  (async()=>{
   ...
   if(l){
    setLessonId(l.id);setGoals(l.learning_goals||"");setPlan(l.plan||"");setStatus(l.status);setStartedAt(l.started_at);setEndedAt(l.ended_at);setCarryNote(l.carry_forward_note||"");setCarriedTo(l.carry_forward_to);
 ...
 const persist=async(overrides?:LessonOverride)=>{
  ...
  setLessonId(result.data.id);setStatus(result.data.status as Lesson["status"]);setStartedAt(result.data.started_at);setEndedAt(result.data.ended_at);setCarriedTo(result.data.carry_forward_to);setCarryNote(result.data.carry_forward_note||"");setMessage("Gemt ✓");setSaving(false);return true;
 };
 ...
 return <main style={{minHeight:"100vh",background:"#f5f3ee",color:"#26342e"}}>
  ...
  <section style={{maxWidth:1080,margin:"0 auto",padding:"30px 24px 80px",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:18,alignItems:"start"}}>
   <div style={{display:"grid",gap:16,minWidth:0}}>
    ...
    {canEdit&&<section style={{...card,background:"#f8f3e7"}}><label style={{fontWeight:900,display:"block"}}>Nåede I ikke det hele?</label><p style={{fontSize:13,color:"#746c5a",margin:"5px 0 10px"}}>Skriv det, der skal fortsætte. Det vises automatisk øverst, når du åbner samme skemabrik næste uge — uden at kopiere planen.</p><textarea value={carryNote} onChange={e=>setCarryNote(e.target.value)} rows={4} style={input} placeholder="Fx: Vi nåede ikke den fælles opsamling. Start næste gang med gruppernes sidste argument."/><div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginTop:11}}><button disabled={saving||!carryNote.trim()} onClick={moveForward} style={{...action,background:"#8b7652",opacity:saving||!carryNote.trim()?.5:1}}>Flyt til {shortDate(nextLessonDate)} →</button>{carriedTo&&<button disabled={saving} onClick={clearCarry} style={{border:"1px solid #cfc5b2",borderRadius:9,padding:"10px 12px",fontWeight:800,background:"white",color:"#6d604b",cursor:"pointer"}}>Fjern videreførsel</button>}</div>{carriedTo&&<small style={{display:"block",marginTop:9,color:"#786a51",fontWeight:800}}>Markeret til {shortDate(carriedTo)}</small>}</section>}
   ...
   <aside style={{display:"grid",gap:14,minWidth:0}}>
    ...
    <section style={{...card,background:"#eef2ed"}}><p style={{fontSize:10,fontWeight:900,letterSpacing:1.4,color:"#718077",margin:0}}>NÆSTE SAMME TIME</p><h3 style={{fontFamily:"Georgia,serif",fontSize:19,margin:"7px 0 10px"}}>{shortDate(nextLessonDate)}</h3><Link href={`/calendar/lesson/${entry.id}?date=${nextLessonDate}`} style={{color:"#486b59",fontWeight:900,textDecoration:"none"}}>Åbn næste lektion →</Link></section>
   </aside>
  </section>
 </main>;
}
