...
export default function CalendarPage(){
 ...
 async function createMeeting(){
  ...
  setSaving(true);setFormError("");const{data:meetingId,error}=await supabase.rpc("create_meeting_atomic",{p_title:title.trim(),p_meeting_type:type,p_starts_at:start.toISOString(),p_ends_at:end.toISOString(),p_room_id:roomId||null,p_student_id:studentMeeting&&studentId?Number(studentId):null,p_internal_user_ids:selected,p_guardian_user_ids:studentMeeting?selectedGuardians:[],p_external_name:externalName.trim()||null,p_external_role:externalRole.trim()||null,p_meeting_leader_user_id:meetingLeader||null,p_minute_taker_user_id:minuteTaker||null,p_agenda:agendaDraft.trim()||null,p_internal_notes:internalNotesDraft.trim()||null});
  if(error||!meetingId){setFormError(error?.message||"Mødet kunne ikke oprettes.");setSaving(false);return}window.location.href=`/calendar/meeting/${meetingId}`;
 }
 ...
 function MeetingForm(){return <section style={{...card,marginTop:16}}>
...
const modeButton=(active:boolean):React.CSSProperties=>({border:active?"1px solid #365044":"1px solid transparent",background:active?"#365044":"transparent",color:active?"white":"#4e5e55",borderRadius:8,padding:"8px 11px",fontWeight:900,cursor:"pointer"});
